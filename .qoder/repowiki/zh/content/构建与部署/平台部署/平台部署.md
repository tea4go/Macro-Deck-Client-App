...
## 更新摘要
**所做更改**
- 新增Windows平台PowerShell部署脚本架构分析
- 更新Windows平台构建流程和环境配置
- 增强Windows平台工具链管理和卸载流程
- 完善Windows平台Android和桌面应用构建脚本
- 新增Windows平台脚本通用函数库分析
...
## 简介
本指南面向Macro-Deck-Client-App的多平台部署，覆盖Android APK构建、iOS IPA构建、Web PWA部署、Capacitor配置影响、Docker容器化以及各平台性能优化与发布注意事项。**新增**Windows平台PowerShell部署脚本架构，提供完整的工具链安装、环境配置和构建流程。内容基于仓库中的实际配置文件与CI工作流，帮助开发者在不同平台上稳定交付高质量应用。

## 项目结构
该工程采用Angular + Capacitor多端统一架构：共享Web前端代码通过Capacitor桥接到原生平台能力；Android与iOS分别有各自的构建脚本与CI工作流；Web端以PWA形式发布，使用Service Worker进行离线缓存；**新增**Windows平台提供PowerShell脚本实现本地开发和构建。
...
- 平台构建脚本：Android使用Gradle与Fastlane，iOS使用CocoaPods与Fastlane，**新增**Windows使用PowerShell脚本实现本地构建。
- PWA清单与Service Worker：提供安装体验、图标集与缓存策略，确保Web端离线可用性。
- Dockerfile：在容器中完成依赖安装与构建，输出静态站点用于部署。
- **新增**Windows PowerShell脚本：提供工具链安装、环境配置、构建和卸载的完整解决方案。
...
## 架构总览
下图展示从源码到各平台产物的关键路径：Web端经Angular构建生成www目录并启用Service Worker；Android/iOS端由Capacitor桥接原生能力并通过各自CI工作流产出APK/AAB/IPA；**新增**Windows平台通过PowerShell脚本实现本地构建和工具链管理。
...
- 构建链路
  ...
  - **新增**Windows PowerShell脚本提供本地构建替代方案。
- 环境隔离
...
## 结论
本指南基于仓库现有配置，给出了Android、iOS、Web和**新增**Windows四端的部署路径与最佳实践。通过Capacitor统一Web与原生能力，结合CI工作流与容器化，可实现高效稳定的多平台交付。**新增的Windows PowerShell脚本系统**提供了完整的本地开发和构建解决方案，包含工具链安装、环境配置、应用构建和卸载的全流程支持。建议在生产环境中完善安全策略（HTTPS、混淆）、证书与密钥管理，并持续监控各平台的性能与用户体验。
...
- 关键配置速查
  ...
  - **新增**Windows：PowerShell脚本、工具链检测、环境配置、构建优化