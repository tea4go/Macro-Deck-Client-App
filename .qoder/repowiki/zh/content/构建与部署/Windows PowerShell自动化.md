...
## 更新摘要
**变更内容**
- **新增Gitee发布令牌配置指导**：在scripts/local/android-signing.ps1.example中增加了详细的Gitee个人访问令牌获取和环境变量设置说明
- **完善多平台发布支持**：增强了Gitee平台的发布功能，提供完整的令牌配置指导
- **改进用户配置体验**：为Android签名脚本示例提供了更清晰的配置指引
...
**新增Gitee发布令牌配置指导** 项目最新更新中为Android签名脚本示例文件增加了详细的Gitee发布令牌配置说明。在scripts/local/android-signing.ps1.example文件中，新增了关于Gitee个人访问令牌的完整配置指导，包括令牌获取链接和环境变量设置方法。这一改进显著提升了Gitee平台发布的易用性和用户体验。
...
- [scripts/windows/build_android_bywin.ps1:1-353](file://scripts/windows/build_android_bywin.ps1#L1-L353)
...
- [scripts/windows/build_android_bywin.ps1:305-353](file://scripts/windows/build_android_bywin.ps1#L305-L353)
...
participant SignConfig as 签名配置管理
participant TokenMgr as 令牌管理
...
BuildScript->>SignConfig : Load-AndroidSigningPs1加载配置
SignConfig->>TokenMgr : 加载GITEE_TOKEN等令牌
...
ReleasePublish->>TokenMgr : 检查GITEE_TOKEN令牌
TokenMgr->>SignConfig : 从配置文件读取令牌
alt Platform=github
BuildScript->>GitHubAPI : GitHub发布如果Platform=github
BuildScript->>GitHubAPI : 检查gh CLI登录状态
GitHubAPI->>GitHubAPI : gh auth status
GitHubAPI->>GitHubAPI : gh release create tag artifacts
GitHubAPI-->>BuildScript : 返回发布结果
else Platform=gitee
BuildScript->>GiteeAPI : Gitee发布如果Platform=gitee
GiteeAPI->>GiteeAPI : 检查GITEE_TOKEN环境变量
GiteeAPI->>GiteeAPI : 创建ReleasePOST API
GiteeAPI->>GiteeAPI : 上传APK附件
GiteeAPI->>GiteeAPI : 上传AAB附件
GiteeAPI-->>BuildScript : 返回发布结果
end
...
PublishScript->>TokenMgr : 加载本地签名配置
TokenMgr->>FileHandler : 读取android-signing.ps1
TokenMgr->>TokenMgr : 提取GITEE_TOKEN环境变量
...
GiteeClient->>TokenMgr : 从配置文件获取令牌
...
subgraph "应用构建脚本"
BuildAndroid[build_android_bywin.ps1<br/>新增-Build选项参数<br/>新增发布功能<br/>新增自动versionCode递增<br/>新增Gitee令牌支持]
...
这些新增功能与现有系统的完美结合，使得整个自动化系统更加完善和强大，为开发者提供了从环境搭建到应用发布的完整解决方案。特别是**最新的Gitee令牌配置指导**，进一步完善了整个多平台发布生态系统，为开发者提供了更加便捷和可靠的发布体验。
