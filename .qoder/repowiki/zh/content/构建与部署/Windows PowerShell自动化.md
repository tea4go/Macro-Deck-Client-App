...
## 更新摘要
**变更内容**
- **JDK 21自动检测和切换功能增强**：在_common.ps1中新增完整的jvms工具集成，支持自动检测当前Java版本并切换到JDK 21
- **智能Java版本管理**：构建前自动检查Java版本，使用jvms工具自动切换到JDK 21，构建完成后或出错后自动恢复原始Java版本
- **简化开发工作流程**：消除手动Java版本管理要求，提供无缝的JDK 21环境切换体验
...
**JDK 21自动检测和切换功能重大增强** 项目最新更新中显著增强了Java版本管理能力。新增的jvms工具集成提供了完整的JDK 21自动检测和切换功能，能够在构建前自动检测当前Java版本，如果检测到不兼容的版本则自动使用jvms工具切换到JDK 21，并在构建完成后或出错后自动恢复原始Java版本。这一改进彻底消除了手动Java版本管理的复杂性，为开发者提供了无缝的Android构建体验。

**前端依赖管理系统增强** 项目同时增强了Node.js项目依赖管理功能。新增的deps工具专门处理前端依赖安装，通过npm install --legacy-peer-deps参数解决peer dependency冲突问题。同时，Enhanced-NodeModules函数现在能够检查关键二进制文件（如.bin/ng.cmd和.bin/ionic.cmd）的存在性，而不仅仅是检查node_modules目录是否存在，确保依赖安装的完整性和可用性。
...
Common[_common.ps1<br/>通用函数库<br/>新增Resolve-JunctionPath函数<br/>新增Java版本验证函数<br/>新增Set-AndroidGradleValue函数<br/>新增Sync-AppVersion版本同步函数<br/>新增Require-WebBuildDeps函数<br/>增强Ensure-NodeModules函数<br/>新增jvms版本切换功能]
subgraph "基础工具管理"
...
subgraph "应用构建脚本"
BuildAndroid[build_android_bywin.ps1<br/>Android应用构建<br/>新增-Build选项参数说明<br/>新增自动versionCode递增<br/>新增发布功能<br/>新增Gitee令牌配置支持<br/>新增JDK 21自动切换]
...
- [scripts/windows/_common.ps1:1-1471](file://scripts/windows/_common.ps1#L1-L1471)
...
- [scripts/windows/build_android_bywin.ps1:1-378](file://scripts/windows/build_android_bywin.ps1#L1-L378)
...
**重大更新** JDK 21自动检测和切换功能经过全面增强，引入了完整的jvms工具集成和智能版本管理机制。

#### **新增** jvms版本切换功能
**重大更新** 在_common.ps1中新增了完整的jvms工具支持：

- **Test-JvmsAvailable函数**：检测jvms命令是否可用
- **Get-JvmsListInfo函数**：解析jvms list输出，获取已安装的Java版本列表和当前版本
- **Find-JvmsVersionMatching函数**：从已安装的jvms版本中查找匹配指定主版本号的版本
- **Set-JvmsVersion函数**：通过jvms use命令切换JDK版本，并从注册表刷新当前进程的PATH/JAVA_HOME环境变量

#### **增强** Java版本验证系统
**重大更新** Java版本验证系统经过重新设计，将版本要求从JDK 17-21范围收紧到特定的JDK 21版本：

- **Assert-JavaForAndroid函数**：新的严格版本验证函数，强制要求JDK 21版本
- **精确版本匹配**：Min=21, Max=21的严格范围验证
- **详细错误信息**：提供具体的版本不兼容原因和解决方案
- **jvms切换指导**：明确指导用户使用jvms use 21命令切换Java版本

#### **新增** 智能版本切换流程
**重大更新** 在Android构建脚本中实现了智能的Java版本切换流程：

- **构建前检测**：在构建开始前检测当前Java版本是否为21
- **自动切换**：如果当前版本不是21且安装了jvms，自动切换到JDK 21
- **版本恢复**：构建完成后或出错后自动恢复到原始Java版本
- **优雅降级**：如果未安装jvms，提供清晰的错误提示和手动切换指导

#### 技术实现特点
- **环境变量同步**：切换后从注册表重新读取PATH和JAVA_HOME，确保当前会话立即生效
- **错误处理**：完善的异常处理和回滚机制
- **用户友好**：详细的进度反馈和操作提示
- **构建流程集成**：与现有的Android构建流程无缝集成

**章节来源**
- [scripts/windows/_common.ps1:917-998](file://scripts/windows/_common.ps1#L917-L998)
- [scripts/windows/_common.ps1:888-909](file://scripts/windows/_common.ps1#L888-L909)
- [scripts/windows/build_android_bywin.ps1:305-325](file://scripts/windows/build_android_bywin.ps1#L305-L325)
- [scripts/windows/build_android_bywin.ps1:372-377](file://scripts/windows/build_android_bywin.ps1#L372-L377)
...
**章节来源**
- [scripts/windows/_common.ps1:1162-1210](file://scripts/windows/_common.ps1#L1162-L1210)
...
**重大更新** 该脚本提供了完整的 Android 应用构建支持，集成了Ionic + Capacitor + fastlane工具链，并新增了综合的-Build选项参数说明、发布功能和JDK 21自动切换功能。

#### **新增** JDK 21自动切换集成
**重大更新** 在构建流程中集成了智能的JDK 21自动切换功能：
...
该功能确保Android构建前具备JDK 21环境，并在构建完成后自动恢复原始Java版本。

#### 新增的综合参数系统
**重大更新** 构建了完整的命令行参数体系：

- **-Build参数**: 构建 release APK/AAB（不发布），versionCode 自动递增
- **-Check参数**: 只检查签名变量、npx、fastlane、JDK 是否就绪，不执行 Web/Android 构建
...
- [scripts/windows/build_android_bywin.ps1:305-378](file://scripts/windows/build_android_bywin.ps1#L305-L378)
...
整个 PowerShell 自动化系统采用分层架构设计，新增了Windows批处理入口层和JDK 21自动切换层，确保了高内聚、低耦合的特性：
...
JdkSwitching[JDK 21自动切换<br/>新增jvms工具集成]
end
subgraph "基础设施层"
Common[通用函数库<br/>新增Resolve-JunctionPath函数<br/>新增Java版本验证<br/>新增Set-AndroidGradleValue函数<br/>新增Sync-AppVersion版本同步函数<br/>新增Android签名配置管理<br/>新增Require-WebBuildDeps函数<br/>增强Ensure-NodeModules函数<br/>新增jvms版本切换功能]
Helpers[辅助工具]
Download[下载引擎]
Registry[注册表操作]
Mirror[RubyGems镜像检测]
...
JvmsIntegration[jvms工具集成<br/>新增JDK版本切换]
end
RunBat --> EnvCheck
RunBat --> Install
RunBat --> Uninstall
...
participant JavaDetection as Java版本检测
participant JvmsManager as jvms管理器
participant VersionSwitcher as 版本切换器
participant ErrorRecovery as 错误恢复
User->>BuildScript : 执行Android构建
BuildScript->>JavaDetection : Get-JavaMajorVersion检测当前版本
JavaDetection-->>BuildScript : 返回当前Java版本
alt 当前版本不是21
BuildScript->>JvmsManager : Test-JvmsAvailable检查jvms可用性
JvmsManager-->>BuildScript : 返回jvms可用性
alt jvms可用
BuildScript->>JvmsManager : Find-JvmsVersionMatching查找JDK 21
JvmsManager-->>BuildScript : 返回JDK 21版本名
BuildScript->>JvmsManager : Set-JvmsVersion切换到JDK 21
JvmsManager-->>BuildScript : 返回切换结果
BuildScript->>ErrorRecovery : 记录原始版本用于恢复
else jvms不可用
BuildScript-->>User : 输出警告提示
end
else 当前版本已是21
BuildScript-->>User : 继续构建流程
end
BuildScript->>BuildScript : 执行Android构建
alt 构建成功
BuildScript->>ErrorRecovery : 检查是否需要恢复版本
ErrorRecovery-->>BuildScript : 返回恢复指令
BuildScript->>JvmsManager : Set-JvmsVersion恢复原始版本
JvmsManager-->>BuildScript : 返回恢复结果
else 构建失败
BuildScript->>ErrorRecovery : 检查是否需要恢复版本
ErrorRecovery-->>BuildScript : 返回恢复指令
BuildScript->>JvmsManager : Set-JvmsVersion恢复原始版本
...
- [scripts/windows/build_android_bywin.ps1:329](file://scripts/windows/build_android_bywin.ps1#L329)
...
**图表来源**
- [scripts/windows/_common.ps1:1192-1210](file://scripts/windows/_common.ps1#L1192-L1210)
- [scripts/windows/build_android_bywin.ps1:332](file://scripts/windows/build_android_bywin.ps1#L332)
...
- [scripts/windows/_common.ps1:1375-1419](file://scripts/windows/_common.ps1#L1375-L1419)
- [scripts/windows/_common.ps1:1320-1361](file://scripts/windows/_common.ps1#L1320-L1361)
...
- [scripts/windows/install_1_base_tools_bywin.ps1:781-800](file://scripts/windows/install_1_base_tools_bywin.ps1#L781-800)
...
VerifyBoot --> CheckJava[检查JDK 17环境]
CheckJava --> JavaOK{JDK 17就绪?}
JavaOK --> |否| InstallJava[安装JDK 17]
...
subgraph "共享依赖"
Common[_common.ps1<br/>新增Resolve-JunctionPath函数<br/>新增Java版本验证函数<br/>新增Set-AndroidGradleValue函数<br/>新增Sync-AppVersion版本同步函数<br/>新增Android签名配置管理<br/>新增Require-WebBuildDeps函数<br/>增强Ensure-NodeModules函数<br/>新增jvms版本切换功能]
end
subgraph "基础工具管理"
...
subgraph "应用构建脚本"
BuildAndroid[build_android_bywin.ps1<br/>新增-Build选项参数<br/>新增发布功能<br/>新增自动versionCode递增<br/>新增Gitee令牌支持<br/>新增JDK 21自动切换]
...
- [scripts/windows/build_android_bywin.ps1:50](file://scripts/windows/build_android_bywin.ps1#L50)
- [scripts/windows/build_web_bywin.ps1:266-268](file://scripts/windows/build_web_bywin.ps1#L266-L268)
...
#### **新增** JDK 21版本验证和自动切换
**重大更新** 新增了严格的JDK版本验证和自动切换功能：

- **Assert-JavaForAndroid函数**: 强制要求JDK 21版本
- **详细错误信息**: 提供具体的版本不兼容原因和解决方案
- **jvms切换指导**: 明确指导用户使用jvms use 21命令切换Java版本
- **Capacitor兼容性**: 基于Capacitor插件toolchain硬编码要求
- **自动切换机制**: 构建前自动检测并切换到JDK 21，构建后自动恢复
- **优雅降级**: 未安装jvms时提供清晰的错误提示和手动切换指导
...
- [scripts/windows/_common.ps1:1375-1419](file://scripts/windows/_common.ps1#L1375-L1419)
- [scripts/windows/_common.ps1:1320-1361](file://scripts/windows/_common.ps1#L1320-L1361)
- [scripts/windows/_common.ps1:1430-1470](file://scripts/windows/_common.ps1#L1430-L1470)
- [scripts/windows/_common.ps1:1162-1210](file://scripts/windows/_common.ps1#L1162-L1210)
- [scripts/windows/_common.ps1:888-909](file://scripts/windows/_common.ps1#L888-L909)
- [scripts/windows/_common.ps1:917-998](file://scripts/windows/_common.ps1#L917-L998)
- [scripts/windows/install_1_base_tools_bywin.ps1:736-808](file://scripts/windows/install_1_base_tools_bywin.ps1#L736-L808)
...
#### **新增** JDK 21版本验证和切换性能优化
**重大更新** 新增的JDK 21版本验证和切换功能具有以下性能特点：

- **快速版本检测**: Get-JavaMajorVersion函数仅执行一次java -version调用
- **精确版本匹配**: Min=21, Max=21的严格范围验证
- **渐进式验证**: 区分未安装、版本过低、版本过高的不同情况
- **jvms工具优化**: 通过jvms use命令进行高效的版本切换
- **环境变量同步**: 切换后立即从注册表刷新PATH和JAVA_HOME
- **优雅恢复**: 构建完成后自动恢复原始版本，避免环境污染
- **用户友好的提示信息**: 提供具体的操作步骤和外部资源链接
- **构建流程集成**: 在Android构建流程中尽早进行Java版本检查和切换
...
# 查找匹配的JDK 21版本
Find-JvmsVersionMatching -MajorVersion 21

# 手动设置jvms版本
Set-JvmsVersion -Version "openjdk-21"
...
**新增JDK 21版本强制验证和自动切换** **重大更新** 新增的Assert-JavaForAndroid函数提供了精确的JDK版本范围验证，强制要求JDK 21版本，确保Android构建环境的兼容性和稳定性。该函数基于Capacitor插件toolchain硬编码要求和Gradle 8.13官方支持范围，为Android构建提供了严格的质量保障。**最关键的改进** 通过jvms工具集成，系统现在能够自动检测当前Java版本并在需要时自动切换到JDK 21，构建完成后自动恢复原始版本，彻底消除了手动Java版本管理的复杂性。
...
15. **JDK 21版本强制验证和自动切换**: **新增** 确保Android构建环境的严格兼容性，提供无缝的Java版本管理体验
...
- **JDK 21版本验证和自动切换**: **新增** 精确的JDK 21版本范围验证和智能版本切换机制
...
**JDK 21版本强制验证和自动切换**的引入标志着系统在Java环境管理方面的重要进步。通过Assert-JavaForAndroid函数和jvms工具集成，系统现在能够严格验证JDK 21版本要求，确保Android构建环境的兼容性。**关键更新** 基于Capacitor插件toolchain硬编码要求和Gradle 8.13官方支持范围，系统提供了精确的版本验证和详细的错误信息。**重要改进** 通过jvms自动切换机制，构建前自动检测并切换到JDK 21，构建后自动恢复原始版本，彻底消除了手动Java版本管理的复杂性。
...
**JDK 21版本强制验证和自动切换**的引入标志着系统在Java环境管理方面的重要进步。通过严格的版本验证和智能的自动切换机制，系统现在能够确保Android构建环境的兼容性，同时提供无缝的Java版本管理体验。这一改进显著提升了构建过程的可靠性，避免了因Java版本不兼容导致的构建失败，彻底消除了手动Java版本管理的复杂性。
