...
## 更新摘要
**变更内容**
- **版本号升级**：versionCode从26升级到28，包括Android build.gradle和所有TypeScript环境配置文件的同步更新
- **三向版本同步系统**：继续正常工作，确保跨平台版本一致性，已适配新的versionCode 28
- **环境变量同步**：所有环境文件中的versionCode字段已更新为28，versionName保持3.0.1不变
- **诊断服务版本显示**：Web平台版本显示功能已更新，正确显示versionCode 28
...
## 简介
本文件系统性梳理 Macro-Deck-Client-App 的构建配置，覆盖 Angular CLI 配置、TypeScript 编译配置、环境变量、Service Worker 配置与构建优化策略。本次更新反映了版本号从26升级到28的版本变更，更新了所有相关配置文件中的versionCode值，确保文档与最新的版本号保持一致。项目现已采用 Angular 19.2.6，配合完整的Windows PowerShell构建脚本体系和Ruby + fastlane工具链架构。

**更新重点**：本次更新特别关注了versionCode从26到28的升级，所有环境配置文件中的versionCode字段已更新为28，Android Gradle配置也相应更新。三向版本同步系统已适配新的versionCode值，确保Android、iOS和Web三个平台的版本一致性。新增的版本同步脚本和增强的诊断服务功能，为开发者提供了更便捷的版本管理体验。
...
- **更新**：versionCode已升级到28，应用于所有环境配置
- **更新**：Android Gradle配置已更新到versionCode 28
...
A --> D["src/environments/*<br/>环境变量配置<br/>versionCode: 28"]
...
- **环境变量配置**：区分开发/生产与原生/Web 版本，通过 fileReplacements 动态替换，**更新**：所有环境文件versionCode字段已更新为28，versionName保持3.0.1不变。
...
- **三向版本同步系统**：以Android Gradle配置为权威来源，自动同步到iOS和Web平台，**更新**：已适配versionCode 28。
- **跨平台版本管理机制**：支持Windows PowerShell和Unix Bash脚本的版本同步。
- **版本号权威来源**：android/app/build.gradle作为唯一的版本号来源，**更新**：versionCode已更新为28，versionName保持3.0.1。
- **版本同步函数**：Sync-AppVersion函数实现iOS和Web环境的版本同步，**更新**：已更新到versionCode 28。
- **独立版本同步脚本**：sync_version_bywin.ps1提供单独的版本同步功能。
- **版本显示增强**：诊断服务中显示与Android一致的版本信息，**更新**：显示versionCode 28。
- **Set-AndroidGradleValue函数**：提供精确的Android Gradle配置写入功能。
- **Read-AndroidGradleValue函数**：提供可靠的版本读取功能。
- **版本同步到诊断服务**：增强Web平台版本显示功能，**更新**：显示versionCode 28。
...
participant Env as "环境配置<br/>src/environments/*<br/>versionCode : 28"
...
participant Gradle as "Gradle 构建系统<br/>versionCode : 28"
participant VersionSync as "版本同步系统<br/>versionCode : 28"
...
VersionSync->>VersionSync : 读取Android版本号versionCode=28, versionName=3.0.1
...
Gradle->>Gradle : 应用统一命名规范versionCode : 28
...
- **默认开发环境**：production=false，webVersion=false，version="3.0.1"，versionCode=28
- **原生生产环境**：production=true，webVersion=false，version="3.0.1"，versionCode=28
- **Web 开发环境**：production=false，webVersion=true，version="3.0.1"，versionCode=28
- **Web 生产环境**：production=true，webVersion=true，version="3.0.1"，versionCode=28
- **文件替换机制**：通过 angular.json 的 fileReplacements 将 src/environments/environment.ts 替换为上述任一文件，实现按环境注入
- **版本号同步**：通过三向版本同步系统自动更新版本号，**更新**：所有环境文件versionCode字段已更新为28，versionName保持3.0.1不变
...
L["统一命名规范<br/>APK/AAB 标准化<br/>versionCode: 28"] --> F
M["Gemfile.lock<br/>版本锁定"] --> F
N["RubyGems官方源<br/>稳定性提升"] --> G
O["三向版本同步系统<br/>versionCode: 28"] --> F
...
- **应用配置**：android/app/build.gradle实现了统一的APK命名规范，**更新**：versionCode已更新为28，versionName保持3.0.1
...
- **优势**：便于版本管理和自动化部署
- **更新**：versionCode 28已应用于命名规范，versionName为3.0.1
...
- **sync_version_bywin.ps1**：**更新**独立版本同步脚本，支持versionCode 28
...
- **versionCode**：应用内部版本号，用于构建和发布，**更新**：versionCode已更新为28
...
```mermaid
flowchart TD
A["读取 Android Gradle 配置<br/>versionCode: 28, versionName: 3.0.1"] --> B["提取 versionCode=28 和 versionName=3.0.1"]
B --> C{"版本号有效？"}
C --> |否| D["跳过版本同步"]
C --> |是| E["同步到 iOS 平台"]
C --> |是| F["同步到 Web 平台"]
E --> G["更新 project.pbxproj"]
F --> H["更新 environment*.ts 文件<br/>versionCode: 28, versionName: 3.0.1"]
...
- **CURRENT_PROJECT_VERSION**：同步versionCode到iOS构建版本，**更新**：已同步为28
- **MARKETING_VERSION**：同步versionName到iOS显示版本，**更新**：保持3.0.1不变

### Web平台版本同步
系统批量更新四个环境配置文件：
- **src/environments/environment.ts**：versionName=3.0.1，versionCode=28
- **src/environments/environment.prod.ts**：versionName=3.0.1，versionCode=28
- **src/environments/environment.web.ts**：versionName=3.0.1，versionCode=28
- **src/environments/environment.web.prod.ts**：versionName=3.0.1，versionCode=28

每个文件都包含version和versionCode两个字段的同步更新，**更新**：所有文件的versionCode已更新为28。
...
.EXAMPLE
  .\sync_version_bywin.ps1 -VersionName 3.1.0 -VersionCode 28
  把版本设为 3.1.0(28) 写回 build.gradle，再同步三端.
.EXAMPLE
...
- **版本号验证**：确保从Android Gradle配置正确读取版本号，**更新**：已验证versionCode=28，versionName=3.0.1
- **平台检测**：检查iOS工程是否存在，不存在时跳过同步
- **文件处理**：使用UTF-8无BOM格式写回文件
- **批量更新**：同时更新四个Web环境配置文件，**更新**：所有文件versionCode已更新为28
- **Set-AndroidGradleValue精确写入**：保留原行缩进，自动处理引号
- **Read-AndroidGradleValue可靠读取**：正则表达式精确匹配版本号
- **版本显示增强**：诊断服务显示versionCode 28版本信息
...
- **sync_version_bywin.ps1**：独立的版本同步脚本，**更新**：支持versionCode 28
- **Set-AndroidGradleValue函数**：提供精确的版本写入功能
- **Read-AndroidGradleValue函数**：提供可靠的版本读取功能
...
participant Gradle as "Gradle 构建<br/>versionCode : 28"
participant Sync as "版本同步"
participant iOS as "iOS 平台"
participant Web as "Web 平台"
Build->>Gradle : 读取 Android 版本号 versionCode=28, versionName=3.0.1
Gradle->>Sync : 提供 versionCode=28, versionName=3.0.1
Sync->>iOS : 更新 CURRENT_PROJECT_VERSION=28
Sync->>iOS : 更新 MARKETING_VERSION=3.0.1
Sync->>Web : 更新 environment.ts (versionCode : 28)
Sync->>Web : 更新 environment.prod.ts (versionCode : 28)
Sync->>Web : 更新 environment.web.ts (versionCode : 28)
Sync->>Web : 更新 environment.web.prod.ts (versionCode : 28)
iOS-->>Build : 版本同步完成
Web-->>Build : 版本同步完成
```
...
- **版本同步优化**：三向版本同步系统确保跨平台版本一致性，**更新**：versionCode 28已同步到所有平台
- **Set-AndroidGradleValue精确写入**：避免版本冲突和写入错误
- **Read-AndroidGradleValue可靠读取**：确保版本数据的准确性
- **版本显示增强**：诊断服务显示同步后的版本信息，**更新**：显示versionCode 28
- **TypeScript包含模式优化**：src/**/*.d.ts包含模式确保类型定义被正确处理，**更新**：提升编译可靠性和开发效率
- **测试文件排除规则**：移除不必要的排除规则，**更新**：简化TypeScript编译配置
- **测试配置优化**：src/**/*.spec.ts包含模式确保测试文件被正确处理，**更新**：改善TypeScript开发体验
...
### 版本同步优化
- **权威来源统一**：以Android Gradle配置为唯一版本号来源，**更新**：versionCode已更新为28
- **自动同步机制**：在构建流程中自动执行版本同步
- **跨平台一致性**：确保Android、iOS和Web平台版本号完全一致，**更新**：所有平台versionCode均为28
...
- **Android构建集成**
  - Gradle配置实现统一的APK命名规范，**更新**：versionCode=28，versionName=3.0.1
  - Fastfile增强AAB重命名功能
...
- **版本同步系统集成**
  ...
  - 诊断服务显示同步后的版本信息，**更新**：显示versionCode 28
- **TypeScript编译优化集成**
...
- **版本同步到诊断服务集成**
  - Web平台版本显示与Android保持一致，**更新**：显示versionCode 28
  - 增强的版本信息展示功能
...
CFG --> ENV["src/environments/*<br/>versionCode: 28"]
...
PS --> VERSIONSYNC["版本同步系统<br/>versionCode: 28"]
...
GRADLE["Gradle 构建系统<br/>versionCode: 28"] --> NAMING["统一命名规范"]
...
VERSIONSYNC --> DIAGNOSTIC["诊断服务版本显示<br/>versionCode: 28"]
DIAGNOSTIC --> WEBENV["Web环境版本显示<br/>versionCode: 28"]
...
- **构建产物命名优化**
  - 统一的APK命名规范简化了版本管理和自动化处理，**更新**：versionCode 28已应用于命名规范
  - AAB重命名功能减少了手动干预的需求
- **依赖版本锁定优化**
...
- **Ruby依赖源稳定性优化**
  ...
  - 提升Ruby依赖获取的可靠性
- **版本同步性能优化**
  - 以Android Gradle配置为权威来源，避免版本冲突，**更新**：versionCode 28已同步
  - 自动同步机制减少手动干预
  - 独立版本同步脚本支持快速对齐
  - 跨平台一致性确保用户体验统一，**更新**：所有平台versionCode均为28
  - Set-AndroidGradleValue精确写入功能提升版本管理效率
  - Read-AndroidGradleValue可靠读取功能确保版本数据准确性
  - 诊断服务版本显示功能提升用户体验，**更新**：显示versionCode 28
- **TypeScript编译性能优化**
  - src/**/*.d.ts包含模式确保类型定义被正确处理，**更新**：提升编译可靠性和开发效率
  - 移除不必要的排除规则，**更新**：简化TypeScript编译配置
  - src/**/*.spec.ts包含模式确保测试文件被正确处理，**更新**：改善TypeScript开发体验
...
- **APK命名问题**
  ...
  - **新增**：验证versionName和versionCode是否正确传递给构建系统，**更新**：versionCode应为28
...
- **版本同步问题**
  - **新增**：检查Android Gradle配置中的versionCode和versionName是否正确，**更新**：versionCode应为28，versionName应为3.0.1
  - **新增**：验证Sync-AppVersion函数是否正确读取版本号
  - **新增**：确认iOS工程的project.pbxproj文件是否存在且可写
  - **新增**：检查Web环境配置文件的同步是否成功，**更新**：所有文件versionCode应为28
  - **新增**：验证版本同步脚本是否正确执行
  - **新增**：检查Set-AndroidGradleValue函数是否正确写入版本
  - **新增**：检查Read-AndroidGradleValue函数是否正确读取版本
  - **新增**：验证版本同步到诊断服务的功能是否正常，**更新**：应显示versionCode 28
- **版本显示问题**
  - **新增**：检查Web平台版本显示是否与Android版本一致，**更新**：应显示versionCode 28
  - **新增**：验证诊断服务的getVersion方法是否正确返回版本信息，**更新**：应返回versionCode 28
  - **新增**：确认environment.ts中的版本号是否正确同步，**更新**：versionCode应为28
- **TypeScript编译问题**
...
## 结论
本项目通过 Angular CLI、TypeScript、Service Worker 与 Capacitor 的协同，实现了跨平台的构建与部署能力。本次versionCode从26升级到28的版本变更反映了项目的持续演进，所有环境配置文件中的versionCode字段已同步更新。项目现已采用 Angular 19.2.6，配套依赖版本同步更新，并建立了完整的 Windows PowerShell 构建脚本体系。

**重要更新**：本次更新特别关注了versionCode从26到28的升级，所有环境配置文件中的versionCode字段已更新为28，Android Gradle配置也相应更新。三向版本同步系统已适配新的versionCode值，确保Android、iOS和Web三个平台的版本一致性。新增的版本同步脚本和增强的诊断服务功能，为开发者提供了更便捷的版本管理体验。

**新增关键特性**：
- **versionCode升级**：从26升级到28，所有环境配置文件versionCode字段已更新为28
- **Android版本更新**：Android Gradle配置中的versionCode更新为28，versionName保持3.0.1不变
- **三向版本同步系统**：已适配versionCode 28，确保跨平台版本一致性
- **独立版本同步脚本**：提供了sync_version_bywin.ps1独立版本同步功能，支持versionCode 28
- **版本号权威来源**：确保Android、iOS和Web平台版本号完全一致，**更新**：所有平台versionCode均为28
- **版本同步函数**：Sync-AppVersion函数实现完整的版本同步流程，**更新**：已更新到versionCode 28
- **跨平台版本管理**：支持Windows PowerShell和Unix Bash脚本的版本同步，**更新**：所有平台versionCode均为28
- **版本显示增强**：诊断服务中显示与Android一致的版本信息，**更新**：显示versionCode 28
- **Set-AndroidGradleValue函数**：提供精确的Android Gradle配置写入功能
- **Read-AndroidGradleValue函数**：提供可靠的版本读取功能
- **统一的APK命名规范**：实现了MacroDeckClient-<版本名>-<版本号>.apk的标准化命名，**更新**：versionCode 28，versionName 3.0.1
...
- **Windows PowerShell 构建**：
  ...
  - **新增**：手动版本设置：`.\scripts\windows\sync_version_bywin.ps1 -VersionName 3.0.1 -VersionCode 28`
- **Unix Bash 构建**：
...
- **环境替换**：通过 fileReplacements 动态切换，**更新**：所有环境文件versionCode均为28
...
- **Web 配置**：专门的 Web/PWA 构建配置集，**更新**：versionCode 28
...
- **PATH同步优化**：通过Sync-PathFromRegistry函数解决Ruby安装后的PATH同步问题
- **增强的命令执行显示**：Install-WingetPackage函数改进了命令执行的可视化反馈
- **统一APK命名**：实现MacroDeckClient-<版本名>-<版本号>.apk的标准化命名，**更新**：versionCode 28，versionName 3.0.1
- **AAB重命名功能**：通过Fastfile增强AAB文件的命名一致性
- **Gemfile.lock依赖管理**：确保Ruby依赖版本的稳定性
- **Google Play部署**：完整的部署流程和前置条件说明
- **RubyGems官方源**：从腾讯云镜像切换回官方RubyGems.org，提高构建稳定性
- **三向版本同步系统**：以Android Gradle配置为权威来源的跨平台版本管理，**更新**：versionCode 28
- **独立版本同步脚本**：提供sync_version_bywin.ps1独立版本同步功能，**更新**：支持versionCode 28
- **版本同步函数**：Sync-AppVersion函数实现完整的版本同步流程，**更新**：已更新到versionCode 28
- **版本显示增强**：诊断服务中显示与Android一致的版本信息，**更新**：显示versionCode 28
- **Set-AndroidGradleValue函数**：提供精确的Android Gradle配置写入功能
- **Read-AndroidGradleValue函数**：提供可靠的版本读取功能
