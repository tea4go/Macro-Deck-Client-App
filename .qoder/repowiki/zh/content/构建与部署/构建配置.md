...
## 更新摘要
**变更内容**
- 新增三向版本同步系统：添加了独立的版本同步脚本和PowerShell版本同步函数
- 实现跨平台版本管理机制：支持Windows PowerShell和Unix Bash脚本的版本同步
- 建立版本号权威来源：以Android Gradle配置为唯一版本号权威源
- 完善版本同步流程：自动同步到iOS和Web环境，保持三端版本一致性
- 增强版本显示功能：在诊断服务中显示与Android一致的版本信息
...
**更新重点**：本次更新特别关注了新增的三向版本同步系统，实现了以Android Gradle配置为权威来源的跨平台版本管理机制。通过独立的版本同步脚本和PowerShell版本同步函数，确保Android、iOS和Web三个平台的版本号保持完全一致，解决了传统构建流程中版本号不一致的问题。
...
- **三向版本同步系统**：以Android Gradle配置为权威来源，自动同步到iOS和Web平台。
- **跨平台版本管理机制**：支持Windows PowerShell和Unix Bash脚本的版本同步。
- **版本号权威来源**：android/app/build.gradle作为唯一的版本号来源。
- **版本同步函数**：Sync-AppVersion函数实现iOS和Web环境的版本同步。
- **独立版本同步脚本**：sync_version_bywin.ps1提供单独的版本同步功能。
- **版本显示增强**：诊断服务中显示与Android一致的版本信息。
...
participant VersionSync as "版本同步系统"
...
VersionSync->>VersionSync : 读取Android版本号
...
- [capacitor.config.ts:6](file://capacitor.config.ts#L6)
- [scripts/windows/_common.ps1:1-200](file://scripts/windows/_common.ps1#L1-L200)
- [scripts/windows/sync_version_bywin.ps1:18-21](file://scripts/windows/sync_version_bywin.ps1#L18-L21)
...
- **版本号同步**：通过三向版本同步系统自动更新版本号

**章节来源**
- [src/environments/environment.ts:1-21](file://src/environments/environment.ts#L1-L21)
- [src/environments/environment.prod.ts:1-11](file://src/environments/environment.prod.ts#L1-L11)
- [src/environments/environment.web.ts:1-11](file://src/environments/environment.web.ts#L1-L11)
- [src/environments/environment.web.prod.ts:1-11](file://src/environments/environment.web.prod.ts#L1-L11)
...
- **应用配置**：android/app/build.gradle实现了统一的APK命名规范
...
- **sync_version_bywin.ps1**：**新增**独立版本同步脚本
...
- [scripts/windows/install_3_fastlane_bywin.ps1:1-69](file://scripts/windows/install_3_fastlane_bywin.ps1#L1-L69)
- [scripts/windows/install_4_android_sdk_bywin.ps1:1-249](file://scripts/windows/install_4_android_sdk_bywin.ps1#L1-L249)
- [scripts/windows/build_android_bywin.ps1:145-157](file://scripts/windows/build_android_bywin.ps1#L145-L157)
...
- **versionCode**：应用内部版本号，用于构建和发布
- **versionName**：应用对外显示的版本号，用于用户界面显示
...
```mermaid
flowchart TD
A["读取 Android Gradle 配置"] --> B["提取 versionCode 和 versionName"]
B --> C{"版本号有效？"}
C --> |否| D["跳过版本同步"]
C --> |是| E["同步到 iOS 平台"]
C --> |是| F["同步到 Web 平台"]
E --> G["更新 project.pbxproj"]
F --> H["更新 environment*.ts 文件"]
...
- **sync_version_bywin.ps1**：独立的版本同步脚本
...
Build->>Gradle : 读取 Android 版本号
Gradle->>Sync : 提供 versionCode/versionName
Sync->>iOS : 更新 CURRENT_PROJECT_VERSION
Sync->>iOS : 更新 MARKETING_VERSION
...
**图表来源**
- [scripts/windows/build_web_bywin.ps1:266-270](file://scripts/windows/build_web_bywin.ps1#L266-L270)
- [scripts/windows/build_android_bywin.ps1:149-151](file://scripts/windows/build_android_bywin.ps1#L149-L151)
- [scripts/windows/sync_version_bywin.ps1:18-21](file://scripts/windows/sync_version_bywin.ps1#L18-L21)
...
- **版本号验证**：确保从Android Gradle配置正确读取版本号
- **平台检测**：检查iOS工程是否存在，不存在时跳过同步
- **文件处理**：使用UTF-8无BOM格式写回文件
- **批量更新**：同时更新四个Web环境配置文件
...
### 版本同步优化
- **权威来源统一**：以Android Gradle配置为唯一版本号来源
- **自动同步机制**：在构建流程中自动执行版本同步
- **跨平台一致性**：确保Android、iOS和Web平台版本号完全一致
...
GRADLE["Gradle 构建系统"] --> NAMING["统一命名规范"]
...
VERSIONSYNC --> DIAGNOSTIC["诊断服务版本显示"]
```
...
- **版本同步问题**
  - **新增**：检查Android Gradle配置中的versionCode和versionName是否正确
  - **新增**：验证Sync-AppVersion函数是否正确读取版本号
  - **新增**：确认iOS工程的project.pbxproj文件是否存在且可写
  - **新增**：检查Web环境配置文件的同步是否成功
  - **新增**：验证版本同步脚本是否正确执行
...
**新增关键特性**：
- **三向版本同步系统**：实现了以Android Gradle配置为权威来源的跨平台版本管理
- **独立版本同步脚本**：提供了sync_version_bywin.ps1独立版本同步功能
- **版本号权威来源**：确保Android、iOS和Web平台版本号完全一致
- **版本同步函数**：Sync-AppVersion函数实现完整的版本同步流程
- **跨平台版本管理**：支持Windows PowerShell和Unix Bash脚本的版本同步
- **版本显示增强**：诊断服务中显示与Android一致的版本信息
- **统一的APK命名规范**：实现了MacroDeckClient-<版本名>-<版本号>.apk的标准化命名
...
这些改进使得项目在Windows平台上的开发体验得到显著提升，特别是在Ruby、Bundler和fastlane工具链的安装和使用方面。新增的三向版本同步系统解决了跨平台版本号不一致的历史问题，通过权威来源和自动同步机制，确保了用户体验的一致性。合理利用配置集、预算限制与缓存策略，可以在保证开发体验的同时，获得高效的生产构建结果。建议在团队协作中统一构建脚本与配置，持续监控包体与缓存命中率，以维持良好的用户体验。
...
- **PATH同步优化**：通过Sync-PathFromRegistry函数解决Ruby安装后的PATH同步问题
- **增强的命令执行显示**：Install-WingetPackage函数改进了命令执行的可视化反馈
- **统一APK命名**：实现MacroDeckClient-<版本名>-<版本号>.apk的标准化命名
- **AAB重命名功能**：通过Fastfile增强AAB文件的命名一致性
- **Gemfile.lock依赖管理**：确保Ruby依赖版本的稳定性
- **Google Play部署**：完整的部署流程和前置条件说明
- **RubyGems官方源**：从腾讯云镜像切换回官方RubyGems.org，提高构建稳定性
- **三向版本同步系统**：以Android Gradle配置为权威来源的跨平台版本管理
- **独立版本同步脚本**：提供sync_version_bywin.ps1独立版本同步功能
- **版本同步函数**：Sync-AppVersion函数实现完整的版本同步流程
- **版本显示增强**：诊断服务中显示与Android一致的版本信息
