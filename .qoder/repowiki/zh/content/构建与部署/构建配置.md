...
- **Android版本更新**：Android Gradle配置中的versionName更新为3.0.1，versionCode更新为9
...
- **默认开发环境**：production=false，webVersion=false，version="3.0.1"，versionCode=9
...
- **应用配置**：android/app/build.gradle实现了统一的APK命名规范，**更新**：版本已更新为3.0.1，versionCode更新为9
...
- **优势**：便于版本管理和自动化部署
- **更新**：版本3.0.1已应用于命名规范，versionCode为9
...
- **versionCode**：应用内部版本号，用于构建和发布，**更新**：版本已更新为9
...
```mermaid
flowchart TD
A["读取 Android Gradle 配置<br/>3.0.1"] --> B["提取 versionCode=9 和 versionName=3.0.1"]
B --> C{"版本号有效？"}
C --> |否| D["跳过版本同步"]
C --> |是| E["同步到 iOS 平台"]
C --> |是| F["同步到 Web 平台"]
...
### Web平台版本同步
系统批量更新四个环境配置文件：
- **src/environments/environment.ts**：版本3.0.1，versionCode=9
- **src/environments/environment.prod.ts**：版本3.0.1，versionCode=9
- **src/environments/environment.web.ts**：版本3.0.1，versionCode=9
- **src/environments/environment.web.prod.ts**：版本3.0.1，versionCode=9
...
Gradle->>Sync : 提供 versionCode=9, versionName=3.0.1
Sync->>iOS : 更新 CURRENT_PROJECT_VERSION=9
...
- **Android构建集成**
  - Gradle配置实现统一的APK命名规范，**更新**：版本3.0.1，versionCode=9
  - Fastfile增强AAB重命名功能
...
- **版本同步问题**
  - **新增**：检查Android Gradle配置中的versionCode和versionName是否正确，**更新**：版本已更新为3.0.1，versionCode=9
...
- **Set-AndroidGradleValue函数**：提供精确的Android Gradle配置写入功能
- **Read-AndroidGradleValue函数**：提供可靠的版本读取功能
- **统一的APK命名规范**：实现了MacroDeckClient-<版本名>-<版本号>.apk的标准化命名，**更新**：版本3.0.1，versionCode=9
...
- **Windows PowerShell 构建**：
  ...
  - **新增**：手动版本设置：`.\scripts\windows\sync_version_bywin.ps1 -VersionName 3.0.1 -VersionCode 9`
- **Unix Bash 构建**：
...
- **PATH同步优化**：通过Sync-PathFromRegistry函数解决Ruby安装后的PATH同步问题
- **增强的命令执行显示**：Install-WingetPackage函数改进了命令执行的可视化反馈
- **统一APK命名**：实现MacroDeckClient-<版本名>-<版本号>.apk的标准化命名，**更新**：版本3.0.1，versionCode=9
- **AAB重命名功能**：通过Fastfile增强AAB文件的命名一致性
