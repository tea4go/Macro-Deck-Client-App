...
## 更新摘要
**变更内容**
- Ruby依赖源从腾讯云镜像切换回官方RubyGems.org，提高构建稳定性
- 新增统一的APK命名规范：MacroDeckClient-<版本名>-<版本号>.apk
- 增强Fastfile的AAB重命名功能，实现与APK命名规范的一致性
- 新增Gemfile.lock确保Ruby依赖版本一致性
- 优化Android构建流程，支持统一的文件命名规范
- 完善Google Play Store部署文档和前置条件说明
...
**更新重点**：本次更新特别关注了Android构建产物的命名规范化，实现了统一的APK命名规范（MacroDeckClient-<版本名>-<版本号>.apk），并通过Fastfile增强了AAB重命名功能，确保所有构建产物的一致性。同时，新增的Gemfile.lock文件确保了Ruby依赖版本的稳定性，为构建流程提供了更好的可预测性。
...
- **build_android_bywin.ps1**：Android 构建脚本（新增签名环境检查）
...
**新增关键特性**：
- **统一的APK命名规范**：实现了MacroDeckClient-<版本名>-<版本号>.apk的标准化命名，提升了版本管理和自动化处理的效率
- **AAB重命名功能**：通过Fastfile增强了AAB文件的命名一致性，确保与APK命名规范保持统一
- **Gemfile.lock依赖管理**：新增的依赖版本锁定文件确保了构建环境的稳定性，避免了因依赖版本变化导致的构建失败
- **Google Play Store部署完善**：提供了完整的部署文档和前置条件说明，简化了应用发布的流程
- **RubyGems官方源稳定性**：从腾讯云镜像切换回官方RubyGems.org，显著提高了构建稳定性，减少了因镜像源不稳定导致的构建失败
