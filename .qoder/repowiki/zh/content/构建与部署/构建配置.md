...
## 更新摘要
**变更内容**
- **版本号升级**：versionCode从37升级到49，包括Android build.gradle和所有TypeScript环境配置文件的同步更新
- **Capacitor诊断日志启用**：在capacitor.config.ts中启用了临时诊断日志行为以辅助开发故障排除
- **三向版本同步系统**：继续正常工作，确保跨平台版本一致性，已适配新的versionCode 49
- **环境变量同步**：所有环境文件中的versionCode字段已更新为49，versionName保持3.0.1不变
- **诊断服务版本显示**：Web平台版本显示功能已更新，正确显示versionCode 49
- **iOS版本信息更新**：Info.plist中的CFBundleShortVersionString和CFBundleVersion已更新
...
## 简介
本文件系统性梳理 Macro-Deck-Client-App 的构建配置，覆盖 Angular CLI 配置、TypeScript 编译配置、环境变量、Service Worker 配置与构建优化策略。本次更新反映了版本号从37升级到49的版本变更，更新了所有相关配置文件中的versionCode值，确保文档与最新的版本号保持一致。项目现已采用 Angular 19.2.6，配合完整的Windows PowerShell构建脚本体系和Ruby + fastlane工具链架构。

**更新重点**：本次更新特别关注了versionCode从37到49的升级，所有环境配置文件中的versionCode字段已更新为49，Android Gradle配置也相应更新。三向版本同步系统已适配新的versionCode值，确保Android、iOS和Web三个平台的版本一致性。新增的Capacitor诊断日志功能和增强的诊断服务功能，为开发者提供了更便捷的故障排除体验。
...
- **更新**：versionCode已升级到49，应用于所有环境配置
- **更新**：Android Gradle配置已更新到versionCode 49
- **更新**：Capacitor诊断日志功能已启用以辅助开发故障排除
- **更新**：三向版本同步系统已适配新的versionCode值

```mermaid
graph TB
A["angular.json<br/>CLI 构建配置"] --> B["tsconfig*.json<br/>TypeScript 编译配置<br/>包含src/**/*.d.ts模式"]
A --> C["ngsw-config.json<br/>Service Worker 配置"]
A --> D["src/environments/*<br/>环境变量配置<br/>versionCode: 49"]
A --> E["capacitor.config.ts<br/>Capacitor 输出目录<br/>诊断日志启用"]
...
- **环境变量配置**：区分开发/生产与原生/Web 版本，通过 fileReplacements 动态替换，**更新**：所有环境文件versionCode字段已更新为49，versionName保持3.0.1不变。
- **Service Worker 配置**：定义缓存组、预取与懒加载策略、更新模式与资源匹配规则。
- **浏览器兼容性**：通过 browserslist 指定目标浏览器集合，影响转译与 polyfill。
- **Capacitor 输出目录**：与构建输出目录保持一致，确保原生应用正确加载静态资源。**更新**：启用了临时诊断日志行为以辅助开发故障排除。
...
- **三向版本同步系统**：以Android Gradle配置为权威来源，自动同步到iOS和Web平台，**更新**：已适配versionCode 49。
- **跨平台版本管理机制**：支持Windows PowerShell和Unix Bash脚本的版本同步。
- **版本号权威来源**：android/app/build.gradle作为唯一的版本号来源，**更新**：versionCode已更新为49，versionName保持3.0.1。
- **版本同步函数**：Sync-AppVersion函数实现iOS和Web环境的版本同步，**更新**：已更新到versionCode 49。
- **独立版本同步脚本**：sync_version_bywin.ps1提供单独的版本同步功能。
- **版本显示增强**：诊断服务中显示与Android一致的版本信息，**更新**：显示versionCode 49。
- **Set-AndroidGradleValue函数**：提供精确的Android Gradle配置写入功能。
- **Read-AndroidGradleValue函数**：提供可靠的版本读取功能。
- **版本同步到诊断服务**：增强Web平台版本显示功能，**更新**：显示versionCode 49。
...
- **iOS版本信息管理**：Info.plist中的版本信息已同步更新。
...
- [capacitor.config.ts:6-11](file://capacitor.config.ts#L6-L11)
- [ionic.config.json:1-10](file://ionic.config.json#L1-L10)
- [scripts/windows/_common.ps1:1-200](file://scripts/windows/_common.ps1#L1-L200)
- [scripts/windows/sync_version_bywin.ps1:1-80](file://scripts/windows/sync_version_bywin.ps1#L1-L80)
- [scripts/windows/install_3_fastlane_bywin.ps1:1-69](file://scripts/windows/install_3_fastlane_bywin.ps1#L1-L69)
...
participant Env as "环境配置<br/>src/environments/*<br/>versionCode : 49"
...
participant Gradle as "Gradle 构建系统<br/>versionCode : 49"
participant VersionSync as "版本同步系统<br/>versionCode : 49"
participant DiagLog as "Capacitor 诊断日志<br/>临时启用"
...
VersionSync->>VersionSync : 读取Android版本号versionCode=49, versionName=3.0.1
...
- [scripts/windows/install_3_fastlane_bywin.ps1:32-63](file://scripts/windows/install_3_fastlane_bywin.ps1#L32-63)
...
- **默认开发环境**：production=false，webVersion=false，version="3.0.1"，versionCode=49
- **原生生产环境**：production=true，webVersion=false，version="3.0.1"，versionCode=49
- **Web 开发环境**：production=false，webVersion=true，version="3.0.1"，versionCode=49
- **Web 生产环境**：production=true，webVersion=true，version="3.0.1"，versionCode=49
- **文件替换机制**：通过 angular.json 的 fileReplacements 将 src/environments/environment.ts 替换为上述任一文件，实现按环境注入
- **版本号同步**：通过三向版本同步系统自动更新版本号，**更新**：所有环境文件versionCode字段已更新为49，versionName保持3.0.1不变
...
- **诊断日志**：**更新**：启用了临时诊断日志行为（loggingBehavior: 'production'）以辅助开发故障排除
...
C --> D["构建输出<br/>www 目录"]
D --> E["Capacitor Server<br/>Android: http<br/>iOS: 自定义方案<br/>诊断日志启用"]
...
L["统一命名规范<br/>APK/AAB 标准化<br/>versionCode: 49"] --> F
M["Gemfile.lock<br/>版本锁定"] --> F
N["RubyGems官方源<br/>稳定性提升"] --> G
O["三向版本同步系统<br/>versionCode: 49"] --> F
...
- [capacitor.config.ts:7-11](file://capacitor.config.ts#L7-L11)
- [scripts/windows/install_3_fastlane_bywin.ps1:32-63](file://scripts/windows/install_3_fastlane_bywin.ps1#L32-63)
- [scripts/windows/install_4_android_sdk_bywin.ps1:155-181](file://scripts/windows/install_4_android_sdk_bywin.ps1#L155-L181)
- [android/app/build.gradle:10-11](file://android/app/build.gradle#L10-L11)
- [android/fastlane/Fastfile:36-47](file://android/fastlane/Fastfile#L36-L47)
...
- **应用配置**：android/app/build.gradle实现了统一的APK命名规范，**更新**：versionCode已更新为49，versionName保持3.0.1
...
- **优势**：便于版本管理和自动化部署
- **更新**：versionCode 49已应用于命名规范，versionName为3.0.1
...
**章节来源**
- [android/Gemfile.lock:1-359](file://android/Gemfile.lock#L1-359)
- [scripts/windows/install_3_fastlane_bywin.ps1:82-110](file://scripts/windows/install_3_fastlane_bywin.ps1#L82-110)
...
- **sync_version_bywin.ps1**：**更新**独立版本同步脚本，支持versionCode 49
...
- [scripts/windows/build_web_bywin.ps1:266-270](file://scripts/windows/build_web_bywin.ps1#L266-270)
...
- **versionCode**：应用内部版本号，用于构建和发布，**更新**：versionCode已更新为49
- **versionName**：应用对外显示的版本号，用于用户界面显示，**更新**：versionName保持3.0.1不变
...
```mermaid
flowchart TD
A["读取 Android Gradle 配置<br/>versionCode: 49, versionName: 3.0.1"] --> B["提取 versionCode=49 和 versionName=3.0.1"]
B --> C{"版本号有效？"}
C --> |否| D["跳过版本同步"]
C --> |是| E["同步到 iOS 平台"]
C --> |是| F["同步到 Web 平台"]
E --> G["更新 project.pbxproj"]
F --> H["更新 environment*.ts 文件<br/>versionCode: 49, versionName: 3.0.1"]
...
**图表来源**
- [scripts/windows/_common.ps1:1285-1330](file://scripts/windows/_common.ps1#L1285-1330)
- [android/app/build.gradle:10-11](file://android/app/build.gradle#L10-L11)

### iOS平台版本同步
系统通过正则表达式更新iOS工程的project.pbxproj文件：
- **CURRENT_PROJECT_VERSION**：同步versionCode到iOS构建版本，**更新**：已同步为49
- **MARKETING_VERSION**：同步versionName到iOS显示版本，**更新**：保持3.0.1不变

### Web平台版本同步
系统批量更新四个环境配置文件：
- **src/environments/environment.ts**：versionName=3.0.1，versionCode=49
- **src/environments/environment.prod.ts**：versionName=3.0.1，versionCode=49
- **src/environments/environment.web.ts**：versionName=3.0.1，versionCode=49
- **src/environments/environment.web.prod.ts**：versionName=3.0.1，versionCode=49

每个文件都包含version和versionCode两个字段的同步更新，**更新**：所有文件的versionCode已更新为49。
...
**章节来源**
- [scripts/windows/_common.ps1:1285-1330](file://scripts/windows/_common.ps1#L1285-1330)
- [scripts/windows/_common.ps1:1230-1240](file://scripts/windows/_common.ps1#L1230-1240)
- [android/app/build.gradle:10-11](file://android/app/build.gradle#L10-L11)
...
.EXAMPLE
  .\sync_version_bywin.ps1 -VersionName 3.1.0 -VersionCode 49
  把版本设为 3.1.0(49) 写回 build.gradle，再同步三端.
.EXAMPLE
...
**章节来源**
- [scripts/windows/_common.ps1:1254-1272](file://scripts/windows/_common.ps1#L1254-1272)
...
- **版本号验证**：确保从Android Gradle配置正确读取版本号，**更新**：已验证versionCode=49，versionName=3.0.1
- **平台检测**：检查iOS工程是否存在，不存在时跳过同步
- **文件处理**：使用UTF-8无BOM格式写回文件
- **批量更新**：同时更新四个Web环境配置文件，**更新**：所有文件versionCode已更新为49
- **Set-AndroidGradleValue精确写入**：保留原行缩进，自动处理引号
- **Read-AndroidGradleValue可靠读取**：正则表达式精确匹配版本号
- **版本显示增强**：诊断服务显示versionCode 49版本信息

**章节来源**
- [scripts/windows/_common.ps1:1254-1330](file://scripts/windows/_common.ps1#L1254-1330)
...
- **sync_version_bywin.ps1**：独立的版本同步脚本，**更新**：支持versionCode 49
- **Set-AndroidGradleValue函数**：提供精确的版本写入功能
- **Read-AndroidGradleValue函数**：提供可靠的版本读取功能
...
participant Gradle as "Gradle 构建<br/>versionCode : 49"
participant Sync as "版本同步"
participant iOS as "iOS 平台"
participant Web as "Web 平台"
Build->>Gradle : 读取 Android 版本号 versionCode=49, versionName=3.0.1
Gradle->>Sync : 提供 versionCode=49, versionName=3.0.1
Sync->>iOS : 更新 CURRENT_PROJECT_VERSION=49
Sync->>iOS : 更新 MARKETING_VERSION=3.0.1
Sync->>Web : 更新 environment.ts (versionCode : 49)
Sync->>Web : 更新 environment.prod.ts (versionCode : 49)
Sync->>Web : 更新 environment.web.ts (versionCode : 49)
Sync->>Web : 更新 environment.web.prod.ts (versionCode : 49)
iOS-->>Build : 版本同步完成
Web-->>Build : 版本同步完成
```
...
**图表来源**
- [scripts/windows/build_web_bywin.ps1:266-270](file://scripts/windows/build_web_bywin.ps1#L266-270)
- [scripts/windows/build_android_bywin.ps1:188-190](file://scripts/windows/build_android_bywin.ps1#L188-190)
- [scripts/windows/sync_version_bywin.ps1:68-79](file://scripts/windows/sync_version_bywin.ps1#L68-79)
...
- **版本同步优化**：三向版本同步系统确保跨平台版本一致性，**更新**：versionCode 49已同步到所有平台
- **Set-AndroidGradleValue精确写入**：避免版本冲突和写入错误
- **Read-AndroidGradleValue可靠读取**：确保版本数据的准确性
- **版本显示增强**：诊断服务显示同步后的版本信息，**更新**：显示versionCode 49
- **TypeScript包含模式优化**：src/**/*.d.ts包含模式确保类型定义被正确处理，**更新**：提升编译可靠性和开发效率
- **测试文件排除规则**：移除不必要的排除规则，**更新**：简化TypeScript编译配置
- **测试配置优化**：src/**/*.spec.ts包含模式确保测试文件被正确处理，**更新**：改善TypeScript开发体验
- **Karma测试配置**：覆盖率报告和现代化测试流程，**更新**：支持Jasmine测试框架
- **Capacitor诊断日志**：临时启用诊断日志功能，辅助开发故障排除
...
### 版本同步优化
- **权威来源统一**：以Android Gradle配置为唯一版本号来源，**更新**：versionCode已更新为49
- **自动同步机制**：在构建流程中自动执行版本同步
- **跨平台一致性**：确保Android、iOS和Web平台版本号完全一致，**更新**：所有平台versionCode均为49
...
### Capacitor诊断日志优化
- **临时诊断功能**：在capacitor.config.ts中启用了临时诊断日志行为，辅助开发故障排除
- **生产环境配置**：loggingBehavior设置为'production'，平衡诊断信息与性能
- **开发体验提升**：提供更详细的运行时信息，便于问题定位和调试
...
- [Gemfile:5](file://Gemfile#L5)
- [android/Gemfile:1](file://android/Gemfile#L1)
- [capacitor.config.ts:10-11](file://capacitor.config.ts#L10-L11)
...
- **运行时集成**
  ...
  - **更新**：Capacitor诊断日志功能已启用，辅助运行时问题诊断
- **PowerShell 脚本集成**
  - _common.ps1 提供通用函数库
...
- **Android构建集成**
  - Gradle配置实现统一的APK命名规范，**更新**：versionCode=49，versionName=3.0.1
  - Fastfile增强AAB重命名功能
...
- **版本同步系统集成**
  ...
  - 诊断服务显示同步后的版本信息，**更新**：显示versionCode 49
- **TypeScript编译优化集成**
...
- **版本同步到诊断服务集成**
  - Web平台版本显示与Android保持一致，**更新**：显示versionCode 49
  - 增强的版本信息展示功能
- **iOS版本管理集成**
  - Info.plist中的版本信息已同步更新
  - project.pbxproj文件通过版本同步系统管理

```mermaid
graph LR
P["package.json<br/>版本3.0.0"] --> CLI["Angular CLI"]
CLI --> CFG["angular.json"]
CFG --> ENV["src/environments/*<br/>versionCode: 49"]
CFG --> TS["tsconfig*.json<br/>包含src/**/*.d.ts模式<br/>简化排除规则"]
TS --> BROWS["browserslistrc"]
CFG --> SW["ngsw-config.json"]
CFG --> WWW["www 输出目录"]
WWW --> CAP["capacitor.config.ts<br/>诊断日志启用"]
CAP --> DIAG["诊断日志<br/>辅助调试"]
...
PS --> VERSIONSYNC["版本同步系统<br/>versionCode: 49"]
...
GRADLE["Gradle 构建系统<br/>versionCode: 49"] --> NAMING["统一命名规范"]
...
VERSIONSYNC --> DIAGNOSTIC["诊断服务版本显示<br/>versionCode: 49"]
DIAGNOSTIC --> WEBENV["Web环境版本显示<br/>versionCode: 49"]
TSOPTIMIZATION["TypeScript编译优化<br/>包含模式<br/>简化排除规则<br/>测试文件处理"] --> TS
KARMA["Karma测试配置<br/>覆盖率报告"] --> TS
IOSINFO["iOS Info.plist<br/>版本信息同步"] --> IOSVER["iOS版本管理"]
IOSVER --> PROJECTPBX["project.pbxproj<br/>版本同步"]
```
...
- [android/fastlane/Matchfile:1-13](file://android/fastlane/Matchfile#L1-L13)
- [ios/App/App/Info.plist:30-33](file://ios/App/App/Info.plist#L30-L33)
- [ios/App/App.xcodeproj/project.pbxproj:1-200](file://ios/App/App.xcodeproj/project.pbxproj#L1-L200)
...
- **构建产物命名优化**
  - 统一的APK命名规范简化了版本管理和自动化处理，**更新**：versionCode 49已应用于命名规范
  - AAB重命名功能减少了手动干预的需求
- **依赖版本锁定优化**
...
- **Ruby依赖源稳定性优化**
  ...
  - 提升Ruby依赖获取的可靠性
- **版本同步性能优化**
  - 以Android Gradle配置为权威来源，避免版本冲突，**更新**：versionCode 49已同步
  - 自动同步机制减少手动干预
  - 独立版本同步脚本支持快速对齐
  - 跨平台一致性确保用户体验统一，**更新**：所有平台versionCode均为49
  - Set-AndroidGradleValue精确写入功能提升版本管理效率
  - Read-AndroidGradleValue可靠读取功能确保版本数据准确性
  - 诊断服务版本显示功能提升用户体验，**更新**：显示versionCode 49
- **TypeScript编译性能优化**
  - src/**/*.d.ts包含模式确保类型定义被正确处理，**更新**：提升编译可靠性和开发效率
  - 移除不必要的排除规则，**更新**：简化TypeScript编译配置
  - src/**/*.spec.ts包含模式确保测试文件被正确处理，**更新**：改善TypeScript开发体验
...
- **APK命名问题**
  ...
  - **新增**：验证versionName和versionCode是否正确传递给构建系统，**更新**：versionCode应为49
...
- **版本同步问题**
  - **新增**：检查Android Gradle配置中的versionCode和versionName是否正确，**更新**：versionCode应为49，versionName应为3.0.1
  - **新增**：验证Sync-AppVersion函数是否正确读取版本号
  - **新增**：确认iOS工程的project.pbxproj文件是否存在且可写
  - **新增**：检查Web环境配置文件的同步是否成功，**更新**：所有文件versionCode应为49
  - **新增**：验证版本同步脚本是否正确执行
  - **新增**：检查Set-AndroidGradleValue函数是否正确写入版本
  - **新增**：检查Read-AndroidGradleValue函数是否正确读取版本
  - **新增**：验证版本同步到诊断服务的功能是否正常，**更新**：应显示versionCode 49
- **版本显示问题**
  - **新增**：检查Web平台版本显示是否与Android版本一致，**更新**：应显示versionCode 49
  - **新增**：验证诊断服务的getVersion方法是否正确返回版本信息，**更新**：应返回versionCode 49
  - **新增**：确认environment.ts中的版本号是否正确同步，**更新**：versionCode应为49
- **TypeScript编译问题**
...
- **Capacitor诊断日志问题**
  - **新增**：检查capacitor.config.ts中的loggingBehavior配置是否正确
  - **新增**：确认诊断日志功能已正确启用
  - **新增**：验证诊断信息是否有助于问题定位
  - **新增**：在生产环境中适当调整日志级别以避免性能影响
...
## 结论
本项目通过 Angular CLI、TypeScript、Service Worker 与 Capacitor 的协同，实现了跨平台的构建与部署能力。本次versionCode从37升级到49的版本变更反映了项目的持续演进，所有环境配置文件中的versionCode字段已同步更新。项目现已采用 Angular 19.2.6，配套依赖版本同步更新，并建立了完整的 Windows PowerShell 构建脚本体系。

**重要更新**：本次更新特别关注了versionCode从37到49的升级，所有环境配置文件中的versionCode字段已更新为49，Android Gradle配置也相应更新。三向版本同步系统已适配新的versionCode值，确保Android、iOS和Web三个平台的版本一致性。新增的Capacitor诊断日志功能和增强的诊断服务功能，为开发者提供了更便捷的故障排除体验。

**新增关键特性**：
- **versionCode升级**：从37升级到49，所有环境配置文件versionCode字段已更新为49
- **Android版本更新**：Android Gradle配置中的versionCode更新为49，versionName保持3.0.1不变
- **三向版本同步系统**：已适配versionCode 49，确保跨平台版本一致性
- **独立版本同步脚本**：提供了sync_version_bywin.ps1独立版本同步功能，支持versionCode 49
- **版本号权威来源**：确保Android、iOS和Web平台版本号完全一致，**更新**：所有平台versionCode均为49
- **版本同步函数**：Sync-AppVersion函数实现完整的版本同步流程，**更新**：已更新到versionCode 49
- **跨平台版本管理**：支持Windows PowerShell和Unix Bash脚本的版本同步，**更新**：所有平台versionCode均为49
- **版本显示增强**：诊断服务中显示与Android一致的版本信息，**更新**：显示versionCode 49
- **Set-AndroidGradleValue函数**：提供精确的Android Gradle配置写入功能
- **Read-AndroidGradleValue函数**：提供可靠的版本读取功能
- **统一的APK命名规范**：实现了MacroDeckClient-<版本名>-<版本号>.apk的标准化命名，**更新**：versionCode 49，versionName 3.0.1
...
- **测试配置优化**：src/**/*.spec.ts包含模式确保测试文件被正确处理，**更新**：改善TypeScript开发体验
- **Karma测试配置**：覆盖率报告和现代化测试流程，**更新**：支持Jasmine测试框架
- **Capacitor诊断日志**：启用了临时诊断日志功能，辅助开发故障排除

这些改进使得项目在Windows平台上的开发体验得到显著提升，特别是在Ruby、Bundler和fastlane工具链的安装和使用方面。新增的三向版本同步系统解决了跨平台版本号不一致的历史问题，通过权威来源和自动同步机制，确保了用户体验的一致性。新增的Set-AndroidGradleValue和Read-AndroidGradleValue函数进一步增强了版本管理的准确性和可靠性。新增的Capacitor诊断日志功能为开发过程中的问题定位提供了有力支持。合理利用配置集、预算限制与缓存策略，可以在保证开发体验的同时，获得高效的生产构建结果。建议在团队协作中统一构建脚本与配置，持续监控包体与缓存命中率，以维持良好的用户体验。
...
- **Windows PowerShell 构建**：
  ...
  - **新增**：手动版本设置：`.\scripts\windows\sync_version_bywin.ps1 -VersionName 3.0.1 -VersionCode 49`
- **Unix Bash 构建**：
...
- **环境替换**：通过 fileReplacements 动态切换，**更新**：所有环境文件versionCode均为49
...
- **Web 配置**：专门的 Web/PWA 构建配置集，**更新**：versionCode 49
...
- **PATH同步优化**：通过Sync-PathFromRegistry函数解决Ruby安装后的PATH同步问题
- **增强的命令执行显示**：Install-WingetPackage函数改进了命令执行的可视化反馈
- **统一APK命名**：实现MacroDeckClient-<版本名>-<版本号>.apk的标准化命名，**更新**：versionCode 49，versionName 3.0.1
- **AAB重命名功能**：通过Fastfile增强AAB文件的命名一致性
- **Gemfile.lock依赖管理**：确保Ruby依赖版本的稳定性
- **Google Play部署**：完整的部署流程和前置条件说明
- **RubyGems官方源**：从腾讯云镜像切换回官方RubyGems.org，提高构建稳定性
- **三向版本同步系统**：以Android Gradle配置为权威来源的跨平台版本管理，**更新**：versionCode 49
- **独立版本同步脚本**：提供sync_version_bywin.ps1独立版本同步功能，**更新**：支持versionCode 49
- **版本同步函数**：Sync-AppVersion函数实现完整的版本同步流程，**更新**：已更新到versionCode 49
- **版本显示增强**：诊断服务中显示与Android一致的版本信息，**更新**：显示versionCode 49
- **Set-AndroidGradleValue函数**：提供精确的Android Gradle配置写入功能
- **Read-AndroidGradleValue函数**：提供可靠的版本读取功能
