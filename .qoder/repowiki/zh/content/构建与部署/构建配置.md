...
- [scripts/unix/build-web.sh](file://scripts/unix/build-web.sh)
- [scripts/unix/build-android.sh](file://scripts/unix/build-android.sh)
- [scripts/unix/build-ios-ipa.sh](file://scripts/unix/build-ios-ipa.sh)
</cite>

## 更新摘要
**变更内容**
- 新增完整的 Windows PowerShell 构建脚本体系，替代原有 CMD 脚本
- 建立跨平台构建脚本统一架构，包含 Windows PowerShell 与 Unix Bash 脚本
- 新增构建脚本组织结构，支持 Web/PWA、Android、Windows 桌面多平台构建
- 增强环境检查与自动修复机制，包括 Node.js 版本验证、AJV 冲突处理
- 完善构建流程标准化，提供统一的构建命令接口
...
7. [跨平台构建脚本体系](#跨平台构建脚本体系)
8. [Web/PWA 构建配置](#webpwa-构建配置)
9. [构建优化策略](#构建优化策略)
10. [依赖关系分析](#依赖关系分析)
11. [性能考虑](#性能考虑)
12. [故障排查指南](#故障排查指南)
...
## 简介
本文件系统性梳理 Macro-Deck-Client-App 的构建配置，覆盖 Angular CLI 配置、TypeScript 编译配置、环境变量、Service Worker 配置与构建优化策略。本次重大更新反映了从 CMD 脚本到 PowerShell 脚本的全面迁移，建立了现代化的 Windows 构建系统，并提供了完整的跨平台构建脚本体系。文档结合实际源码进行可视化说明，帮助开发者快速理解与高效维护构建流程。
...
- [ngsw-config.json:1-31](file://ngsw-config.json#L1-L31)
- [capacitor.config.ts:6](file://capacitor.config.ts#L6)
- [scripts/windows/build_web_bywin.ps1:200-293](file://scripts/windows/build_web_bywin.ps1#L200-L293)
...
### 脚本概述
PowerShell 构建脚本提供了完整的 Web/PWA 构建自动化流程，支持环境检查、依赖管理、构建执行和错误处理。该脚本体系现已完全替代原有的 CMD 脚本，提供更强大的功能和更好的用户体验。

### 主要功能特性
- **环境检查**：验证 Node.js 版本（要求 v18.19.1+ / v20.11.1+ / v22+）、npm 可用性
- **依赖管理**：自动安装 node_modules，处理 ajv 版本冲突问题
- **构建执行**：支持 build、dev、serve、check 四种模式
- **错误处理**：完善的错误捕获与用户友好的错误信息
- **进度反馈**：详细的构建进度和状态输出
- **跨平台支持**：提供 Windows 平台专用的构建优化

### 构建模式详解
- **build 模式**：执行 Web/PWA 生产构建，产物输出到 www/
- **dev 模式**：启动本地开发服务器，支持热重载
- **serve 模式**：用 http-server 静态托管 www\ 构建产物
- **check 模式**：仅进行环境检查，不执行构建

### 高级功能特性
- **AJV 版本自动修复**：自动检测并修复 Angular 19 兼容性问题
- **构建缓存清理**：智能处理 TypeScript 编译缓存问题
- **环境变量加载**：支持 .env 文件的环境变量注入
- **内存优化**：针对 Windows 平台的内存使用优化
...
- **install_2_c_compile_bywin.ps1**：C/C++ 编译工具安装脚本
- **_common.ps1**：PowerShell 通用函数库
...
| 脚本名称 | 平台 | 主要功能 | 环境要求 |
|---------|------|----------|----------|
| build_web_bywin.ps1 | Windows | Web/PWA 构建、环境检查、AJV 修复 | Node.js 18+/20+/22+ |
...
### 跨平台一致性设计
- **统一的构建接口**：所有脚本都支持相同的构建配置参数
- **共享的工具函数**：通过 common.sh 提供跨平台的通用功能
- **标准化的输出格式**：所有脚本都提供一致的进度和结果输出
- **相似的错误处理机制**：跨平台的错误报告和处理方式
...
- [scripts/unix/build-web.sh:1-12](file://scripts/unix/build-web.sh#L1-L12)
- [scripts/unix/build-android.sh:1-151](file://scripts/unix/build-android.sh#L1-L151)
- [scripts/unix/build-ios-ipa.sh:1-25](file://scripts/unix/build-ios-ipa.sh#L1-L25)
- [scripts/shared/common.sh:1-46](file://scripts/shared/common.sh#L1-L46)
...
## 结论
本项目通过 Angular CLI、TypeScript、Service Worker 与 Capacitor 的协同，实现了跨平台的构建与部署能力。本次重大更新反映了从 CMD 脚本到 PowerShell 脚本的全面现代化迁移，建立了完整的跨平台构建脚本体系。新增的 Windows PowerShell 构建脚本、Unix Bash 构建脚本以及共享脚本库，进一步增强了项目的可维护性和开发效率。合理利用配置集、预算限制与缓存策略，可以在保证开发体验的同时，获得高效的生产构建结果。建议在团队协作中统一构建脚本与配置，持续监控包体与缓存命中率，以维持良好的用户体验。
