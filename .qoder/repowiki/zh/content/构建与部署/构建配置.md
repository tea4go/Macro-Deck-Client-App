...
## 更新摘要
**变更内容**
- **TypeScript编译配置优化**：改进src/**/*.ts包含模式和测试文件排除规则，提升编译可靠性和开发效率
- **版本升级**：从3.0.0升级到3.0.1，更新所有环境配置中的版本号
- **Android版本更新**：Android Gradle配置中的versionName更新为3.0.1，versionCode保持5
- **三向版本同步系统**：更新到3.0.1版本，确保跨平台版本一致性
- **环境变量同步**：所有环境文件中的version字段已更新为3.0.1
...
```mermaid
graph TB
A["angular.json<br/>CLI 构建配置"] --> B["tsconfig*.json<br/>TypeScript 编译配置<br/>包含src/**/*.ts模式"]
...
CC["TypeScript包含模式优化<br/>src/**/*.ts包含<br/>测试文件排除"] --> B
DD["测试配置优化<br/>src/**/*.d.ts包含"] --> B
...
- **TypeScript 编译配置**：基础编译选项、严格模式、模块解析策略、目标语言级别与库支持等。**更新**：改进src/**/*.ts包含模式和测试文件排除规则，提升编译可靠性和开发效率。
- **环境变量配置**：区分开发/生产与原生/Web 版本，通过 fileReplacements 动态替换，**更新**：所有环境文件版本号已更新为3.0.1。
...
- **版本号权威来源**：android/app/build.gradle作为唯一的版本号来源，**更新**：版本已更新为3.0.1。
...
- **TypeScript包含模式优化**：src/**/*.ts包含模式确保所有源文件被正确编译，**更新**：提升编译可靠性和开发效率。
- **测试文件排除规则**：src/**/*.spec.ts和src/test.ts排除规则避免测试文件参与生产构建，**更新**：提升构建性能和安全性。
- **测试配置优化**：src/**/*.d.ts包含模式确保类型定义被正确处理，**更新**：改善TypeScript开发体验。
...
- [tsconfig.app.json:3-20](file://tsconfig.app.json#L3-L20)
...
participant TS as "TypeScript 编译器<br/>包含src/**/*.ts模式"
...
CLI->>TS : 读取 tsconfig*.json 并编译<br/>包含src/**/*.ts模式<br/>排除测试文件
TS-->>CLI : 生成 JS/CSS/资源
...
- **应用配置（tsconfig.app.json）**
  ...
  - **更新**：改进包含模式，显式声明src/**/*.d.ts和src/**/*.ts包含模式
  - **更新**：添加src/**/*.spec.ts和src/test.ts排除规则，避免测试文件参与生产构建
  - **更新**：提升编译可靠性和开发效率
- **测试配置（tsconfig.spec.json）**
  - 继承基础配置
  - 配置Jasmine测试框架
  - **更新**：添加src/**/*.d.ts包含模式，确保类型定义被正确处理
...
- **默认开发环境**：production=false，webVersion=false，version="3.0.1"，versionCode=5
...
- **版本号同步**：通过三向版本同步系统自动更新版本号，**更新**：所有环境文件版本号已更新为3.0.1
...
S["TypeScript包含模式优化<br/>src/**/*.ts包含"] --> C
T["测试文件排除规则<br/>避免测试文件参与构建"] --> C
U["测试配置优化<br/>src/**/*.d.ts包含"] --> C
...
- **应用配置**：android/app/build.gradle实现了统一的APK命名规范，**更新**：版本已更新为3.0.1
...
- **Read-AndroidGradleValue函数**：提供可靠的版本读取功能
- **TypeScript包含模式优化**：提升编译可靠性和开发效率，**更新**：src/**/*.ts包含模式确保所有源文件被正确编译
- **测试文件排除规则**：避免测试文件参与生产构建，**更新**：src/**/*.spec.ts和src/test.ts排除规则提升构建性能和安全性
- **测试配置优化**：改善TypeScript开发体验，**更新**：src/**/*.d.ts包含模式确保类型定义被正确处理
...
- **versionCode**：应用内部版本号，用于构建和发布，**更新**：版本已更新为5
...
```mermaid
flowchart TD
A["读取 Android Gradle 配置<br/>3.0.1"] --> B["提取 versionCode=5 和 versionName=3.0.1"]
B --> C{"版本号有效？"}
C --> |否| D["跳过版本同步"]
C --> |是| E["同步到 iOS 平台"]
C --> |是| F["同步到 Web 平台"]
...
### Web平台版本同步
系统批量更新四个环境配置文件：
- **src/environments/environment.ts**：版本3.0.1
- **src/environments/environment.prod.ts**：版本3.0.1
- **src/environments/environment.web.ts**：版本3.0.1
- **src/environments/environment.web.prod.ts**：版本3.0.1
...
**章节来源**
- [scripts/windows/_common.ps1:1194-1238](file://scripts/windows/_common.ps1#L1194-L1238)
- [scripts/windows/_common.ps1:1139-1148](file://scripts/windows/_common.ps1#L1139-L1148)
- [android/app/build.gradle:10-11](file://android/app/build.gradle#L10-L11)
...
**章节来源**
- [scripts/windows/_common.ps1:1162-1180](file://scripts/windows/_common.ps1#L1162-L1180)
...
**章节来源**
- [scripts/windows/_common.ps1:1162-1238](file://scripts/windows/_common.ps1#L1162-L1238)
...
Gradle->>Sync : 提供 versionCode=5, versionName=3.0.1
Sync->>iOS : 更新 CURRENT_PROJECT_VERSION=5
...
### TypeScript编译优化
- **包含模式优化**：src/**/*.ts包含模式确保所有源文件被正确编译，**更新**：提升编译可靠性和开发效率
- **测试文件排除**：src/**/*.spec.ts和src/test.ts排除规则避免测试文件参与生产构建，**更新**：提升构建性能和安全性
- **类型定义处理**：src/**/*.d.ts包含模式确保类型定义被正确处理，**更新**：改善TypeScript开发体验
- **编译性能提升**：精确的包含和排除规则减少不必要的文件扫描和编译

**章节来源**
- [angular.json:47-118](file://angular.json#L47-118)
- [ngsw-config.json:4-29](file://ngsw-config.json#L4-29)
- [tsconfig.app.json:12-19](file://tsconfig.app.json#L12-L19)
...
- **Android构建集成**
  - Gradle配置实现统一的APK命名规范，**更新**：版本3.0.1
  - Fastfile增强AAB重命名功能
...
- **TypeScript编译优化集成**
  - src/**/*.ts包含模式确保所有源文件被正确编译，**更新**：提升编译可靠性和开发效率
  - src/**/*.spec.ts和src/test.ts排除规则避免测试文件参与生产构建，**更新**：提升构建性能和安全性
  - src/**/*.d.ts包含模式确保类型定义被正确处理，**更新**：改善TypeScript开发体验
...
CFG --> TS["tsconfig*.json<br/>包含src/**/*.ts模式<br/>排除测试文件"]
...
TSOPTIMIZATION["TypeScript编译优化<br/>包含模式<br/>排除规则<br/>类型定义处理"] --> TS
...
- [tsconfig.app.json:1-21](file://tsconfig.app.json#L1-21)
...
- **TypeScript编译性能优化**
  ...
  - src/**/*.spec.ts和src/test.ts排除规则避免测试文件参与生产构建，**更新**：提升构建性能和安全性
  - src/**/*.d.ts包含模式确保类型定义被正确处理，**更新**：改善TypeScript开发体验
  - 精确的包含和排除规则减少不必要的文件扫描和编译，**更新**：提升编译性能
- **测试配置性能优化**
...
- **版本同步问题**
  - **新增**：检查Android Gradle配置中的versionCode和versionName是否正确，**更新**：版本已更新为3.0.1
...
- **TypeScript编译问题**
  - **新增**：检查src/**/*.ts包含模式是否正确识别所有源文件
  - **新增**：验证src/**/*.spec.ts和src/test.ts排除规则是否生效
  - **新增**：确认src/**/*.d.ts包含模式是否正确处理类型定义
  - **新增**：检查TypeScript编译配置是否与Angular CLI配置兼容
- **测试配置问题**
  - **新增**：验证Karma测试配置是否正确加载Jasmine框架
  - **新增**：检查覆盖率报告配置是否正确生成
  - **新增**：确认测试文件排除规则是否阻止测试文件参与生产构建
  - **新增**：验证TypeScript测试配置是否正确处理类型定义
...
**新增关键特性**：
- **版本3.0.1升级**：所有环境配置文件版本号已更新为3.0.1
- **Android版本更新**：Android Gradle配置中的versionName更新为3.0.1，versionCode保持5
...
- **Set-AndroidGradleValue函数**：提供精确的Android Gradle配置写入功能
- **Read-AndroidGradleValue函数**：提供可靠的版本读取功能
- **统一的APK命名规范**：实现了MacroDeckClient-<版本名>-<版本号>.apk的标准化命名，**更新**：版本3.0.1
...
- **TypeScript编译优化**：改进src/**/*.ts包含模式和测试文件排除规则，**更新**：提升编译可靠性和开发效率
- **测试配置优化**：src/**/*.d.ts包含模式确保类型定义被正确处理，**更新**：改善TypeScript开发体验
- **Karma测试配置**：覆盖率报告和现代化测试流程，**更新**：支持Jasmine测试框架
...
- **Windows PowerShell 构建**：
  ...
  - **新增**：手动版本设置：`.\scripts\windows\sync_version_bywin.ps1 -VersionName 3.0.1 -VersionCode 5`
- **Unix Bash 构建**：
...
- **PATH同步优化**：通过Sync-PathFromRegistry函数解决Ruby安装后的PATH同步问题
- **增强的命令执行显示**：Install-WingetPackage函数改进了命令执行的可视化反馈
- **统一APK命名**：实现MacroDeckClient-<版本名>-<版本号>.apk的标准化命名，**更新**：版本3.0.1
- **AAB重命名功能**：通过Fastfile增强AAB文件的命名一致性
...
- **Set-AndroidGradleValue函数**：提供精确的Android Gradle配置写入功能
- **Read-AndroidGradleValue函数**：提供可靠的版本读取功能
- **TypeScript编译优化**：src/**/*.ts包含模式确保所有源文件被正确编译，**更新**：提升编译可靠性和开发效率
- **测试文件排除规则**：src/**/*.spec.ts和src/test.ts排除规则避免测试文件参与生产构建，**更新**：提升构建性能和安全性
- **测试配置优化**：src/**/*.d.ts包含模式确保类型定义被正确处理，**更新**：改善TypeScript开发体验
- **Karma测试配置**：覆盖率报告和现代化测试流程，**更新**：支持Jasmine测试框架
