...
- **新增** 文本发送功能的完整国际化支持，包含模态框标题、占位符文本、按钮标签等7个新翻译键值
- **重大更新** 新增剪贴板读取功能的完整国际化支持，包括"从剪贴板读入"按钮的中英文翻译
- **更新** Send Text模态框按钮标签优化，从"Keyboard input"/"Paste (clipboard)"改进为更准确的"Send keystrokes"/"Send text"
...
- 完善的翻译资源文件，涵盖所有界面文本的本地化，现包含157个翻译键值
...
- **新增** 文本发送功能的完整国际化支持，包含模态框标题、占位符文本、按钮标签等7个新翻译键值
- **重大更新** 新增剪贴板读取功能的完整国际化支持，为用户提供便捷的文本输入方式
- **更新** Send Text模态框按钮标签优化，提供更准确的动作描述："Send keystrokes"和"Send text"
...
R[send-text-modal.component.ts] --> S[send-text-modal.component.html]
T[deck.page.ts] --> R
U[protocol2-messages.ts] --> R
end
subgraph "翻译资源"
D --> N[英文翻译 - 157行]
E --> O[中文翻译 - 157行]
...
X[文本发送功能] --> Y["sendText.*" 8个新键值]
Z[菜单项国际化] --> AA["menu.sendText" 新增]
BB[剪贴板读取功能] --> CC["pasteFromClipboard" 新增]
DD[按钮标签优化] --> EE["sendKeyboard" → "Send keystrokes"]
FF[剪贴板发送优化] --> GG["sendClipboard" → "Send text"]
end
subgraph "语言设置管理"
HH[SettingsService] --> II[语言持久化存储]
JJ[SettingsModalComponent] --> KK[语言切换界面]
KK --> LL[实时语言应用]
MM[选择框国际化] --> NN[动态按钮文本]
NN --> OO[okText/cancelText属性]
end
subgraph "连接状态管理"
PP[WebSocketService] --> QQ[LoadingService]
QQ --> RR[ConnectingComponent]
RR --> SS[连接等待提示]
SS --> TT["等待主机接受连接..."]
end
subgraph "文本发送功能"
UU[SendTextModalComponent] --> VV[文本输入模态框]
VV --> WW[键盘输入按钮]
VV --> XX[剪贴板粘贴按钮]
VV --> YY[剪贴板读取按钮]
WW --> ZZ["sendText.sendKeyboard → Send keystrokes"]
XX --> AAA["sendText.sendClipboard → Send text"]
YY --> BBB["sendText.pasteFromClipboard"]
CCC[DeckPage] --> UUU
CCC --> DDD["menu.sendText"]
end
```
...
### 文本发送服务
**新增** SendTextModalComponent组件，提供文本发送功能的国际化支持，包含键盘输入、剪贴板粘贴和剪贴板读取三种输入方式。
...
FF[DeckPage.openTextInput] --> GG[SendTextModalComponent]
GG --> HH[sendText.title 翻译]
HH --> II[模态框标题显示]
JJ[文本输入] --> KK[sendText.placeholder 翻译]
KK --> LL[占位符文本显示]
MM[键盘输入按钮] --> NN[sendText.sendKeyboard → Send keystrokes]
NN --> OO[按钮标签显示]
PP[剪贴板粘贴按钮] --> QQ[sendText.sendClipboard → Send text]
QQ --> RR[按钮标签显示]
SS[剪贴板读取按钮] --> TT[sendText.pasteFromClipboard 翻译]
TT --> UU[按钮标签显示]
VV[菜单项] --> WW[menu.sendText 翻译]
WW --> XX[菜单标签显示]
```
...
- [loading.service.ts:37-48](file://src/app/services/loading/loading.service.ts#L37-L48)
- [deck.page.ts:62-68](file://src/app/pages/deck/deck.page.ts#L62-L68)
- [send-text-modal.component.html:6](file://src/app/pages/deck/modals/send-text-modal/send-text-modal.component.html#L6-L6)
- [send-text-modal.component.html:13](file://src/app/pages/deck/modals/send-text-modal/send-text-modal.component.html#L13-L13)
- [send-text-modal.component.html:22](file://src/app/pages/deck/modals/send-text-modal/send-text-modal.component.html#L22-L22)
- [send-text-modal.component.html:26](file://src/app/pages/deck/modals/send-text-modal/send-text-modal.component.html#L26-L26)
- [send-text-modal.component.html:30](file://src/app/pages/deck/modals/send-text-modal/send-text-modal.component.html#L30-L30)
- [deck.page.html:22](file://src/app/pages/deck/deck.page.html#L22-L22)
...
**新增** SendTextModalComponent组件提供了完整的文本发送功能国际化支持，现已扩展支持剪贴板读取功能。
...
class DeckPage {
- modalController : ModalController
+ openTextInput() Promise~void~
}
class Protocol2Messages {
+ getSendTextMessage(text : string) object
+ getSendTextClipboardMessage(text : string) object
}
SendTextModalComponent --> ModalController : "使用"
SendTextModalComponent --> WebsocketService : "使用"
SendTextModalComponent --> Protocol2Messages : "生成消息"
DeckPage --> SendTextModalComponent : "创建模态框"
```
...
sequenceDiagram
participant User as 用户
participant Menu as DeckPage
participant Modal as SendTextModalComponent
participant WS as WebsocketService
participant Proto as Protocol2Messages
User->>Menu : 点击"发送文本"菜单项
Menu->>Modal : 创建并显示模态框
Modal->>Modal : 加载保存的文本
User->>Modal : 输入文本内容
User->>Modal : 点击"键盘输入"按钮
Modal->>Proto : getSendTextMessage(text)
Proto-->>Modal : 生成键盘输入消息
Modal->>WS : send(message)
WS-->>Modal : 发送成功
Modal->>Modal : 关闭模态框
User->>Modal : 点击"剪贴板粘贴"按钮
Modal->>Proto : getSendTextClipboardMessage(text)
Proto-->>Modal : 生成剪贴板消息
Modal->>WS : send(message)
WS-->>Modal : 发送成功
Modal->>Modal : 关闭模态框
User->>Modal : 点击"从剪贴板读入"按钮
Modal->>Modal : navigator.clipboard.readText()
Modal->>Modal : 填充文本到输入框
```

**图表来源**
- [deck.page.ts:62-68](file://src/app/pages/deck/deck.page.ts#L62-L68)
- [send-text-modal.component.ts:34-60](file://src/app/pages/deck/modals/send-text-modal/send-text-modal.component.ts#L34-L60)
...
**新增** 文本发送功能提供了完整的国际化界面支持，现已扩展支持剪贴板读取功能。
...
#### 文本输入区域
- **英文界面**：占位符显示 "Enter text to send..."
- **中文界面**：占位符显示 "输入要发送的文本..."

#### 发送按钮
- **键盘输入按钮**：**更新**
  - 英文："Send keystrokes"（原"Keyboard input"）
  - 中文："发送按键"（原"键盘输入"）
- **剪贴板粘贴按钮**：**更新**
  - 英文："Send text"（原"Paste (clipboard)"）
  - 中文："发送文本"（原"粘贴板粘贴"）
- **剪贴板读取按钮**：**新增**
  - 英文："Paste from clipboard"
  - 中文："从剪贴板读入"
...
**重大更新** 新增的剪贴板读取功能为用户提供了更便捷的文本输入方式：

#### 功能特点
- **一键读取**：点击"从剪贴板读入"按钮即可自动读取系统剪贴板内容
- **错误处理**：当浏览器或原生环境不支持剪贴板读取时，静默忽略错误
- **用户体验**：无需手动复制粘贴，提升文本输入效率

#### 技术实现
- **API使用**：使用 `navigator.clipboard.readText()` API 读取剪贴板文本
- **异步处理**：采用 async/await 模式处理异步读取操作
- **兼容性**：在不支持的浏览器环境中优雅降级

### Send Text模态框UI优化

**更新** Send Text模态框的按钮标签得到了显著优化，提供更准确的动作描述：

#### 按钮标签改进
- **键盘输入按钮**：从"Keyboard input"优化为"Send keystrokes"，更准确地描述发送按键动作
- **剪贴板粘贴按钮**：从"Paste (clipboard)"优化为"Send text"，强调发送功能而非粘贴操作
- **用户体验**：新的标签更符合用户预期，减少了操作歧义

#### 界面布局优化
- **顶部工具栏**：保留了取消按钮和剪贴板读取图标按钮
- **主内容区**：保持文本输入区域的完整性
- **底部工具栏**：两个主要发送按钮采用块级布局，提供更好的触摸体验
...
**新增** 项目实现了完整的翻译资源管理，包含157行翻译条目覆盖所有界面功能。
...
#### 英文翻译文件 (en.json)
包含 157 行翻译条目，覆盖以下主题：
- **common**：通用词汇（cancel、save、close、ok、confirm、details等）
- **settings**：设置相关界面文本
- **menu**：菜单项（包含新增的sendText）
- **sendText**：**新增** 文本发送功能专用翻译（title、placeholder、sendKeyboard、sendClipboard、pasteFromClipboard）
- **home**：主页内容
- **addConnection**：连接设置
...
#### 中文翻译文件 (zh.json)
包含 157 行翻译条目，对应英文文件的完整中文翻译。

### 新增文本发送功能翻译键值

**新增** sendText 命名空间下新增了5个翻译键值：

- **title**：模态框标题
  - 英文："Send text"
  - 中文："发送文本"
- **placeholder**：文本输入占位符
  - 英文："Enter text to send..."
  - 中文："输入要发送的文本..."
- **sendKeyboard**：**更新** 键盘输入按钮
  - 英文："Send keystrokes"（原"Keyboard input"）
  - 中文："发送按键"（原"键盘输入"）
- **sendClipboard**：**更新** 剪贴板粘贴按钮
  - 英文："Send text"（原"Paste (clipboard)"）
  - 中文："发送文本"（原"粘贴板粘贴"）
- **pasteFromClipboard**：**新增** 剪贴板读取按钮
  - 英文："Paste from clipboard"
  - 中文："从剪贴板读入"

这些键值被 SendTextModalComponent 组件使用，提供完整的文本发送功能国际化支持。

### 新增菜单项翻译键值

**新增** menu 命名空间下新增了 sendText 翻译键值：

- **sendText**：菜单项标签
  - 英文："Send text"
  - 中文："发送文本"

这个键值被 DeckPage 组件的菜单界面使用，提供文本发送功能的菜单入口。

### 新增通用按钮文本翻译键值

**新增** common 命名空间下新增了 confirm 和 details 翻译键值：
...
这些键值为通用按钮文本，可在多个组件中复用。

### 新增连接状态翻译键值

**新增** connection 命名空间下新增了 waitingForAccept 翻译键值：

- **waitingForAccept**：连接等待提示
...
**章节来源**
- [en.json:1-157](file://src/assets/i18n/en.json#L1-L157)
- [zh.json:1-157](file://src/assets/i18n/zh.json#L1-L157)
...
Y[文本发送功能] --> Z[SendTextModalComponent]
Z --> AA[sendText.* 翻译键值]
BB[菜单项国际化] --> CC[menu.sendText 翻译键值]
DD[剪贴板读取功能] --> EE[navigator.clipboard.readText]
EE --> FF[浏览器API]
GG[按钮标签优化] --> HH["Send keystrokes/Send text"]
HH --> II[用户体验提升]
```
...
- [send-text-modal.component.ts:1-60](file://src/app/pages/deck/modals/send-text-modal/send-text-modal.component.ts#L1-L60)
- [deck.page.html:22](file://src/app/pages/deck/deck.page.html#L22-L22)
- [send-text-modal.component.ts:44](file://src/app/pages/deck/modals/send-text-modal/send-text-modal.component.ts#L44-L44)
...
CC --> DD[选择框国际化]
EE[DeckPage.openTextInput] --> FF[SendTextModalComponent]
FF --> GG[sendText.* 翻译键值]
GG --> HH[文本发送界面国际化]
II[菜单项] --> JJ[menu.sendText 翻译键值]
JJ --> KK[菜单国际化]
LL[pasteFromClipboard功能] --> MM[navigator.clipboard.readText]
MM --> NN[剪贴板读取]
NN --> OO[文本填充到输入框]
PP[按钮标签优化] --> QQ[sendKeyboard/sendClipboard更新]
QQ --> RR[用户体验提升]
```
...
1. **本地存储优化**：使用 localStorage 缓存最近输入的文本，减少重复输入
2. **模态框懒加载**：SendTextModalComponent 仅在需要时创建，避免不必要的内存占用
3. **消息生成优化**：Protocol2Messages 静态方法快速生成发送消息，减少计算开销
4. **翻译键值缓存**：sendText.* 翻译键值在首次使用时加载，后续使用直接缓存

### 剪贴板读取功能性能

**重大更新** 新增剪贴板读取功能的性能优化：

1. **异步操作**：使用 async/await 模式处理剪贴板读取，避免阻塞主线程
2. **错误处理**：在不支持的浏览器环境中静默忽略错误，不影响用户体验
3. **内存管理**：读取的文本内容仅在组件生命周期内使用，及时释放内存
4. **API调用优化**：仅在用户主动点击"从剪贴板读入"按钮时调用剪贴板API

### Send Text模态框UI优化性能

**更新** Send Text模态框UI优化的性能考虑：

1. **按钮标签优化**：更准确的按钮标签减少了用户的认知负担，提升了交互效率
2. **界面布局优化**：块级按钮布局提供了更好的触摸体验，特别是在移动设备上
3. **图标优化**：保留必要的图标元素，同时保持界面的简洁性
4. **响应式设计**：优化后的界面在不同屏幕尺寸下都能提供良好的用户体验
...
#### 文本发送功能国际化问题
**症状**：文本发送模态框显示英文而非本地化文本
**原因**：sendText.* 翻译键值缺失或组件未正确导入 TranslatePipe
**解决方案**：
1. 检查 en.json 和 zh.json 中 sendText.* 键值是否存在（title、placeholder、sendKeyboard、sendClipboard、pasteFromClipboard）
2. 验证 SendTextModalComponent 是否正确导入了 TranslatePipe
3. 确认 send-text-modal.component.html 中使用了正确的翻译键值语法
4. 检查 deck.page.ts 中 openTextInput 方法是否正确创建模态框

#### 菜单项国际化问题
**症状**：菜单中的"发送文本"选项显示英文而非本地化文本
**原因**：menu.sendText 翻译键值缺失或菜单组件未正确使用翻译
**解决方案**：
1. 检查 en.json 和 zh.json 中 menu.sendText 键值是否存在
2. 验证 deck.page.html 中是否正确使用了 {{ 'menu.sendText' | translate }}
3. 确认 DeckPage 组件导入了 TranslatePipe

#### 剪贴板读取功能问题
**症状**：点击"从剪贴板读入"按钮无响应或报错
**原因**：浏览器不支持剪贴板读取API或权限不足
**解决方案**：
1. 检查浏览器是否支持 `navigator.clipboard.readText()` API
2. 确认用户已授予剪贴板访问权限
3. 验证错误处理逻辑是否正确捕获异常
4. 在不支持的浏览器环境中，功能应静默忽略错误而不影响其他功能

#### Send Text模态框按钮标签问题
**症状**：Send Text模态框的按钮标签显示不正确或不是最新的优化版本
**原因**：翻译键值未更新或组件模板未正确绑定
**解决方案**：
1. 检查 en.json 和 zh.json 中 sendText.sendKeyboard 和 sendText.sendClipboard 键值是否为最新优化版本
2. 验证 send-text-modal.component.html 中是否正确使用了更新的翻译键值
3. 确认按钮标签已从"Keyboard input"/"Paste (clipboard)"更新为"Send keystrokes"/"Send text"
4. 检查组件是否正确导入了 TranslatePipe 并使用了正确的翻译语法
...
- [websocket.service.ts:169](file://src/app/services/websocket/websocket.service.ts#L169-L169)
- [settings-modal.component.ts:108-110](file://src/app/pages/shared/modals/settings-modal/settings-modal.component.ts#L108-L110)
- [send-text-modal.component.ts:13](file://src/app/pages/deck/modals/send-text-modal/send-text-modal.component.ts#L13-L13)
- [send-text-modal.component.html:6](file://src/app/pages/deck/modals/send-text-modal/send-text-modal.component.html#L6-L6)
- [send-text-modal.component.html:13](file://src/app/pages/deck/modals/send-text-modal/send-text-modal.component.html#L13-L13)
- [send-text-modal.component.html:22](file://src/app/pages/deck/modals/send-text-modal/send-text-modal.component.html#L22-L22)
- [send-text-modal.component.html:26](file://src/app/pages/deck/modals/send-text-modal/send-text-modal.component.html#L26-L26)
...
- **新增** 文本发送功能的完整国际化支持，包含模态框标题、占位符文本、按钮标签等8个新翻译键值，为用户提供了完整的多语言文本发送体验
- **重大更新** 新增剪贴板读取功能的完整国际化支持，包括"从剪贴板读入"按钮的中英文翻译，为用户提供了更便捷的文本输入方式
- **更新** Send Text模态框按钮标签优化，从"Keyboard input"/"Paste (clipboard)"改进为更准确的"Send keystrokes"/"Send text"，提供了更清晰的操作指导

该系统为用户提供了良好的国际化体验，同时保持了代码的可维护性和扩展性。通过持续的功能完善和质量保证机制，项目正逐步实现对全球开发者的全面支持。最新的优化进一步提升了中文用户的使用体验，体现了项目对本地化细节的关注和对用户体验的重视。新增的连接等待状态提示功能、选择框弹窗国际化改进、文本发送功能的完整国际化支持以及剪贴板读取功能的加入，使得用户在各种交互场景中都能获得一致的本地化体验，进一步改善了整体的用户体验。Send Text模态框的按钮标签优化更是体现了对用户操作习惯的深入理解，让界面交互更加直观和高效。
