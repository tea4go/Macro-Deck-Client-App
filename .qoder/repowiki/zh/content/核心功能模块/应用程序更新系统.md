...
- **最新增强**：SettingsModalComponent实现了Android特定的更新源预检查保存机制，确保用户选择的更新源在检查更新前立即生效
...
- **改进**：支持手动检查更新和智能源切换
- **最新增强**：SettingsModalComponent实现了Android特定的更新源预检查保存机制，确保用户选择的GitHub/Gitee源在检查更新时立即生效，无需先手动保存设置

系统的核心特点包括：
- 基于 GitHub API 和 Gitee API 的双源版本检查
- 用户可配置的更新源选择（GitHub/Gitee）
- **新增**：Android平台特定的更新源预检查保存机制
- 自动解析 APK 资产文件名格式
...
M --> N[更新源选择界面]
M --> O[手动检查更新按钮]
M --> P[Android特定预检查保存机制]
end
subgraph "支持服务"
Q[src/app/services/diagnostic/] --> R[diagnostic.service.ts]
S[src/app/services/i18n/] --> T[i18n.service.ts]
U[src/app/services/] --> V[其他服务]
end
subgraph "国际化资源"
W[src/assets/i18n/] --> X[zh.json]
V --> Y[en.json]
X --> Z[中文翻译]
Y --> AA[英文翻译]
end
subgraph "应用入口"
AB[src/app/app.component.ts] --> AC[应用初始化]
AC --> AD[Service Worker集成]
AC --> AE[更新检查逻辑]
end
B --> Q
B --> H
...
- [settings-modal.component.ts:1-217](file://src/app/pages/shared/modals/settings-modal/settings-modal.component.ts#L1-L217)
...
**最新增强**：集成了Android特定的更新源预检查保存机制，确保用户选择的更新源在检查更新前立即生效。

**新增功能**：
- 更新源选择下拉菜单（GitHub/Gitee）
- 手动检查更新按钮
- **新增**：Android平台特定的更新设置界面
- **新增**：实时更新源配置保存机制
- **最新增强**：`checkForUpdate()`方法中实现了预检查保存逻辑，确保用户选择的更新源在检查更新前立即持久化
...
- [settings-modal.component.ts:72-95](file://src/app/pages/shared/modals/settings-modal/settings-modal.component.ts#L72-L95)
...
subgraph "业务逻辑层"
H[UpdateService] --> I[GitHub API 通信]
H --> J[Gitee API 通信]
H --> K[版本比较逻辑]
H --> L[APK 下载处理]
H --> M[Release 说明解析]
N[SettingsService] --> O[更新源配置管理]
N --> P[跳过版本管理]
N --> Q[其他设置管理]
end
subgraph "平台服务层"
R[DiagnosticService] --> S[平台检测]
R --> T[设备信息]
R --> U[运行时版本获取]
V[Capacitor Plugins] --> W[App 插件]
V --> X[文件系统]
V --> Y[文件打开器]
end
subgraph "Service Worker 层"
Z[SwUpdate Service] --> AA[版本更新检测]
Z --> BB[自动激活机制]
Z --> CC[页面刷新触发]
end
subgraph "外部服务"
DD[GitHub API] --> EE[Releases Endpoint]
FF[Gitee API] --> GG[Releases Endpoint]
HH[GitHub Releases] --> II[APK 资产文件]
JJ[Gitee Releases] --> KK[APK 资产文件]
end
A --> H
A --> Z
H --> R
...
3. **新增**：用户在设置中修改更新源后，点击检查更新时立即保存配置
4. 检查结果决定是否显示更新提示
5. 用户可选择在设置中修改更新源
6. 用户可选择手动检查更新
7. 用户选择更新选项
...
**最新增强**：SettingsModalComponent 实现了Android特定的更新源预检查保存机制。

#### Android预检查保存机制

这是本次更新的核心改进，解决了用户在选择更新源后立即检查更新时的配置同步问题：

```mermaid
sequenceDiagram
participant User as 用户
participant Settings as SettingsModal
participant Storage as SettingsService
participant Update as UpdateService
User->>Settings : 选择更新源为GitHub
User->>Settings : 点击"检查更新"
Settings->>Storage : setUpdateSource('github')
Storage-->>Settings : 配置已保存
Settings->>Update : checkForUpdate(false)
Update->>Storage : getUpdateSource()
Storage-->>Update : 返回'github'
Update->>Update : 使用GitHub API检查更新
Update-->>Settings : 返回检查结果
...
**新增**：`checkForUpdate()`方法中的预检查保存逻辑：

1. **步骤1**：在执行更新检查前，先将当前选择的更新源保存到持久化存储
2. **步骤2**：调用UpdateService进行实际的更新检查
3. **步骤3**：根据检查结果显示相应的提示信息

这种设计确保了即使用户没有点击"保存"按钮，他们选择的更新源也会在检查更新时立即生效。

#### 更新源配置管理

| 方法 | 功能 | 参数 | 返回值 |
|------|------|------|--------|
| `checkForUpdate` | 手动检查更新 | 无 | Promise<void> |
| `saveSettings` | 保存所有设置 | 无 | Promise<void> |
| `loadCurrentSettings` | 加载当前设置 | 无 | Promise<void> |

**章节来源**
- [settings-modal.component.ts:72-95](file://src/app/pages/shared/modals/settings-modal/settings-modal.component.ts#L72-L95)
- [settings-modal.component.ts:122-149](file://src/app/pages/shared/modals/settings-modal/settings-modal.component.ts#L122-L149)
...
4. **新增**：Android平台特定的预检查保存机制确保选择立即生效
...
**新增**：SettingsModalComponent中的Android平台特定优化：
- 仅在Android平台上显示更新源选择界面
- 在检查更新前自动保存更新源配置
- 确保用户选择的更新源立即生效
...
subgraph "平台依赖"
P[DiagnosticService] --> Q[Platform]
P --> R[Device]
P --> S[App]
T[I18nService] --> U[TranslateService]
end
subgraph "Service Worker 依赖"
V[SwUpdate] --> W[VERSION_READY 事件]
V --> X[activateUpdate 方法]
V --> Y[checkForUpdate 方法]
end
A --> P
A --> G
H --> A
...
3. **跨平台兼容**：统一的API调用接口
4. **Android特定优化**：更新源预检查保存机制减少用户操作步骤
...
**症状**：用户选择更新源后点击检查更新，但使用的是之前的源
**原因**：旧版本中需要先保存设置才能生效
**解决方案**：
1. **已解决**：最新版本已实现预检查保存机制
2. 确保使用的是最新版本的应用
3. 如果问题仍然存在，重启应用后重试
...
8. **测试Android预检查保存机制**：验证更新源选择后立即生效的功能
...
7. **增强的运行时版本检测机制**
8. **优化的版本信息显示格式**
9. **最新增强**：Android平台特定的更新源预检查保存机制，提升用户体验
...
- **平台特定的性能优化和资源管理**
- **新增**：Android平台特定的预检查保存机制优化

### 最新改进亮点

**Android平台特定的更新源预检查保存机制**是本系统最新的重大改进：
- **即时生效**：用户选择更新源后，点击检查更新时立即生效，无需先保存设置
- **用户体验提升**：减少了用户的操作步骤，提升了操作的流畅性
- **配置一致性**：确保检查更新时使用的更新源与用户选择的一致
- **平台优化**：仅在Android平台启用此优化，保持跨平台兼容性
...
该更新系统为 Macro Deck 客户端提供了可靠的版本管理能力，确保用户能够及时获得最新的功能和修复。最新的改进包括完整的GitHub和Gitee双源支持、增强的SettingsService配置管理、优化的版本信息显示格式，以及最新的Android平台特定更新源预检查保存机制，进一步增强了系统的稳定性和用户体验。这些改进确保了应用在各种平台环境下都能稳定运行，并提供一致且高效的用戶体验。