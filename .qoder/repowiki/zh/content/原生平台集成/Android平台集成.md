...
- **更新**：src/app/services/update：应用更新服务实现
- **更新**：src/app/pages/shared/modals/update-modal：更新模态框组件
- **新增**：fastlane：自动化构建和发布配置
...
Android --> Fastlane["fastlane/Fastfile"]
...
- **更新**：应用更新服务：基于GitHub Releases的自动更新机制，支持APK下载、安装和版本跳过功能
- **更新**：版本管理系统：统一的versionCode和versionName管理，支持跨平台版本同步
- **新增**：Capacitor插件：集成@capacitor-community/file-opener和@capacitor/filesystem插件，支持文件操作和系统安装器调用
- **新增**：Fastlane自动化：完整的CI/CD流水线，支持自动版本递增和签名构建
...
## 架构概览
应用采用Capacitor架构，原生层（MainActivity）承载系统UI与生命周期，WebView承载前端页面，通过Capacitor桥接原生能力。**更新**的版本管理系统通过Fastlane和PowerShell脚本协调跨平台版本同步，确保Android、iOS和Web平台的版本一致性。**新增**的应用程序更新功能通过UpdateService协调HTTP客户端、文件系统和文件打开器插件，实现完整的应用内更新流程。
...
FL["Fastfile<br/>自动化构建"]
SV["sync_version_bywin.ps1<br/>版本同步"]
end
subgraph "更新功能"
...
- 权限声明：
  ...
  - **更新**：应用安装：REQUEST_INSTALL_PACKAGES（用于触发系统安装器）
...
- **权威源**：android/app/build.gradle作为唯一版本权威
- **同步工具**：PowerShell脚本sync_version_bywin.ps1
- **目标平台**：iOS (project.pbxproj) 和 Web (environment*.ts)
- **自动化**：Fastlane集成CI/CD流水线
...
Fastlane --> Release["Release构建<br/>签名打包"]
```
...
- [Fastfile:15-23](file://android/fastlane/Fastfile#L15-L23)
...
- 组件耦合：
  ...
  - **更新**：UpdateService依赖HttpClient、Filesystem、FileOpener和SettingsService
- 外部依赖：
  - Capacitor核心与插件生态提供跨平台能力
  - AndroidX库与测试框架保障兼容性与质量
  - **更新**：@capacitor-community/file-opener和@capacitor/filesystem插件
- 可能的循环依赖：
  - 未发现直接循环依赖；各模块通过settings.gradle与capacitor.settings.gradle间接关联
  - **更新**：更新服务通过插件间接依赖，形成松耦合设计
- **新增**：版本管理依赖：
  - Fastlane与Gradle集成进行自动化构建
  - PowerShell脚本与Windows环境集成
  - CI/CD流水线与版本同步工具链
...
- **更新**：SDK版本兼容性问题：
  ...
  - 验证构建工具链与IDE版本的兼容性
- **新增**：版本管理故障排除：
  - versionCode不同步：检查sync_version_bywin.ps1脚本执行权限
...
- 构建、调试与发布流程（建议步骤）：
  ...
  - **更新**：版本同步：使用sync_version_bywin.ps1确保三端版本一致性
- 权限管理最佳实践：
  - 仅申请必要权限，遵循最小权限原则
  - 在运行时动态申请敏感权限（如相机、网络状态、安装包）
  - 提供权限说明与用户引导，提升透明度
  - **更新**：REQUEST_INSTALL_PACKAGES权限需要用户明确授权
- 安全配置建议：
  - 生产环境禁用明文HTTP，改为HTTPS
  - 对深度链接添加域名验证与参数校验
  - 定期审查第三方插件与依赖的安全公告
  - **更新**：更新功能仅在可信网络环境下使用GitHub API
- 性能优化建议：
  - 减少不必要的系统UI可见性设置
  - 合理配置WebView与缓存策略
  - 使用数据绑定与懒加载优化UI渲染
  - **更新**：利用新SDK的性能优化特性
  - **更新**：minSdkVersion 23确保支持现代Android设备的最佳性能
  - **新增**：版本同步工具的性能优化
- **更新**：平台兼容性建议：
